...
import type { ComplianceFileUploaderProps } from 'src/types/compliance'

const FileUpload = ({ name, file, onFileUpload }: ComplianceFileUploaderProps) => {
  const auth = useAuth() as any
  const imgPath = auth?.userData?.settings?.DEFAULT_IMAGE_MASTER // Get image paths from user data

  const getFileIcon = () => {
    const fileName = ((file as any)?.name || (file as any)?.file_original_name || '').toLowerCase()
  ...
  return (
    <Box
      sx={{
        border: (file as any)?.file_original_name || (file as any)?.name ? '1px solid #C3CEC7' : '1px dashed #C3CEC7',
        padding: '13px',
        display: 'flex',
        alignItems: 'center',
        cursor: 'pointer',
        ...
        maxWidth: '215px',
        height: '51px',
        background: (file as any)?.file_original_name || (file as any)?.name ? '#F2FFF8' : '#fff'
      }}
    >
      {(file as any)?.file_original_name === null || file === null ? (
        <>
          <img
        ...
        >
          ...
          >
            ...
            >
              {(file as any)?.name || (file as any)?.file_original_name}
            </Typography>
          </Box>
