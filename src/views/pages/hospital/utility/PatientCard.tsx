...
const PatientCard = ({ patientData, animalData, loading, refetch, category }: PatientCardProps) => {
  ...
  const [openEditPatientDrawer, setOpenEditPatientDrawer] = useState<boolean>(false)
  const [openAddAnimalDrawer, setOpenAddAnimalDrawer] = useState<boolean>(false)
  ...
  const [assessmentDrawerOpen, setAssessmentDrawerOpen] = useState<boolean>(false)
  const [healthStatus, setHealthStatus] = useState<string>(patientData?.health_status || 'stable')

  const admissionData: Array<{ type: string; value: any }> = [
  ...
  const handleHealthStatusChange = async (e: SelectChangeEvent<string>) => {
    ...
    try {
      ...
      const response: any = await updateAnimalHealthStatus(payload)
      if (response?.success) {
            ...
            <Grid container spacing={8} alignItems='stretch'>
              ...
              >
                {loading ? (
                  ...
                  <AnimalCard
                    data={animalData}
                    onWeightClick={() => setAssessmentDrawerOpen(true)}
                    maxWidth={'auto'}
                  />
                )}
              </Grid>
...
const StyledTypography = styled(Typography)<{ fontWeight?: number | string; fontSize?: string | number }>(
