...
const SurgeryRecordTemplateList = ({
  ...
  // 🔹 NEW: callback from parent (TemplateSection) to refetch templates after update/delete
  onTemplatesUpdated = () => {}
}) => {
  const theme = useTheme()
  const [searchValue, setSearchValue] = useState('')
  ...
  const filteredTemplates = useMemo(() => {
    ...
    return list.filter(template => {
      const title = template?.title?.toLowerCase() || template?.name?.toLowerCase() || ''
      const description = template?.description?.toLowerCase() || ''
      const category = template?.category?.toLowerCase() || ''
  ...
  // Footer "Close" button — just closes the drawer
  const handleEditTemplate = () => {
    setOpenSurgeryTemplateDrawer(false)
  }

  // Open Edit popup when clicking pencil on card
  const handleEditTemplateFromPencil = template => {
    setEditingTemplate(template)
    setOpenEditPopup(true)
  }

  // 🔹 UPDATE template logic (called by EditTemplateForm on submit)
  const handleUpdateTemplate = useCallback(
    async formData => {
      ...
      try {
        ...
        payload.append('id', editingTemplate.id) // ✅ Correct
        payload.append('template_name', formData.name || '')
        payload.append('description', formData.description || '') // ✅ Now will update

        const response = await updateTemplate(editingTemplate.id, payload)
  ...
  // 🔹 DELETE template logic (called by EditTemplateForm & card delete)
