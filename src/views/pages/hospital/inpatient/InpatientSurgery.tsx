...
const getRichTextHtmlValue = (value: any): string => {
  if (!value) return ''
  ...
  if (value?.delta?.ops) {
    try {
      const text = value.delta.ops.map((op: any) => (typeof op.insert === 'string' ? op.insert : '')).join('')
...
const formatTimeValue = (time: any, date?: any): string => {
  if (!time) return '--'
...
const getDurationLabel = (detail: any): string => {
  if (!detail) return '--'
...
const parseProcedurePerformed = (value: any): string[] => {
  if (!value) return []
  if (Array.isArray(value)) return value.map(String).filter((item: string) => item.trim() !== '')
...
const getRecordIdentifier = (record: any): string => {
...
const getRecordCode = (record: any): string => {
  if (!record || typeof record !== 'object') return ''
...
function InpatientSurgery({ hospitalCaseId, medicalRecordId, patientDischarged = false, category }: InpatientSurgeryProps) {
  ...
  const activeRecord = useMemo(() => {
    ...
    if (activeSurgeryId) {
      const found = surgeryRecords.find((record: any) => getRecordIdentifier(record) === activeSurgeryId)
  ...
  }, [activeSurgeryId, activeSurgeryRecordId])
  ...
  const handleViewAnesthesiaDetails = useCallback(() => {
    ...
    const query: any = { tab: 'anesthesia' }
    if (medicalRecordId) {
      query.medical_record_id = medicalRecordId
    }
  ...
  const surgeryDetailItems = useMemo(() => {
    ...
    const secondarySurgeons =
      detail.secondary_surgeons?.length > 0
        ? detail.secondary_surgeons.map((surgeon: any) => surgeon.user_full_name).join(', ')
        : '--'
  ...
  const anesthesiaInfo = useMemo(() => {
    const detail: any = activeDetail?.anaesthesia_detail || {}
  ...
  const renderTabContent = () => {
    ...
    return surgeryRecords.map((record: any, index: number) => {
      ...
      <Box sx={{ display: 'flex', flexDirection: 'column', gap: '12px' }}>
        ...
        <Grid sx={{ px: '8px' }} container spacing={4}>
          {Array.from({ length: 4 }).map((_, idx) => (
            <Grid size={{ xs: 6, md: 3 }} key={`basic-skeleton-${idx}`}>
        ...
        <Box sx={{ px: '8px', display: 'flex', flexDirection: 'column', gap: '24px' }}>
          <Grid container spacing={4}>
            {Array.from({ length: 3 }).map((_, idx) => (
              <Grid size={{ xs: 12, sm: 6, md: 4 }} key={`surgery-skeleton-${idx}`}>
                  ...
                  {procedurePerformedList.length > 0 && (
                    <>
                      ...
                      <Box component='ul' sx={{ ml: '-8px', mt: 0, mb: 1 }}>
                        {procedurePerformedList.map((item: string, idx: number) => (
                          <li key={`${item}-${idx}`}>
