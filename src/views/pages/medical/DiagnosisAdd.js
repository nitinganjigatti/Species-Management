...
const DiagnosisAddComponents = props => {
  ...
  useEffect(() => {
    ...
    if (editParams.med_cat_id !== null) {
      ...
      setValue(
        'type',
        diagnosisOpt.find(option => option.value === editParams.type)
      )
    }
          ...
          >
            <Box sx={{ gap: 2, display: 'flex', flexDirection: 'row', alignItems: 'center' }}>
              ...
              <Typography variant='h6'>
                {editParams?.med_cat_id !== null ? 'Edit Diagnosis' : 'Add Diagnosis'}
              </Typography>
            </Box>
                ...
                <Controller
                  ...
                  render={({ field: { value, onChange, onBlur } }) => (
                    <Autocomplete
                      ...
                      isOptionEqualToValue={(option, val) => option.val === val?.value}
                      onChange={(event, newValue) => {
                        onChange(newValue)
                      }}
                      onBlur={onBlur} // Ensure the onBlur event is handled
                      renderInput={params => (
                        <TextField
                          {...params}
                          label='Type *'
                          ...
                          helperText={errors?.type?.message} // Display the error message below the input
                        />
                      )}
                    />
                  )}
              ...
              <Box sx={{ display: 'flex', alignItems: 'center' }}>
                ...
                >
                  <LoadingButton fullWidth variant='contained' type='submit' size='large' loading={loading}>
                    {editParams?.med_cat_id !== null ? 'Update Diagnosis' : 'Add Diagnosis'}
                  </LoadingButton>
                </Box>
              </Box>
            </Box>
...
export default DiagnosisAddComponents
