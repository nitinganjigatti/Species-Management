...
const schema = yup.object().shape({
  name: yup.string().required('GST Slab Name is Required'),
...
const AddGstSlabs = props => {
  // ** Props
  const { addEventSidebarOpen, handleSidebarClose, handleSubmitData, resetForm, submitLoader } = props
        ...
        <form autoComplete='off' onSubmit={!submitLoader ? handleSubmit(onSubmit) : null}>
          ...
          <FormControl fullWidth sx={{ mb: 6 }}>
            <Controller
              ...
              rules={{ required: true }}
              render={({ field: { value, onChange } }) => (
                <TextField label='Tax' value={value} onChange={onChange} name='tax' error={Boolean(errors.tax)} />
              )}
            />
            {errors.title && <FormHelperText sx={{ color: 'error.main' }}>{error.name.message}</FormHelperText>}
          </FormControl>
