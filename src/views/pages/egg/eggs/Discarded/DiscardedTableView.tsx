...
const DiscardedTableView: FC<any> = ({
  tabValue,
  setFilterList,
  filterList,
  ...
  const fetchTableData = useCallback(
    async (sort: any, q: any, selectedFiltersOptions: any = {}, filterByNurseryId: any) => {
      try {
        setLoading(true)
  ...
  const searchTableData = useCallback(
    debounce(async (sort: any, q: any) => {
      setSearchValue(q)
      try {
        await fetchTableData(sort, q, selectedFiltersOptions, filterByNurseryId)
      } catch (error) {
        console.error(error)
      }
    }, 1000),
