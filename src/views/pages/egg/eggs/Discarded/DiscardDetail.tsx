...
const DiscardDetail: FC<any> = ({ setDetailDrawer, detailDrawer, eggDiscardedId, fetchTableData }) => {
  ...
  const getEggListSummary = async (id: any) => {
    const params = {
  ...
  const GetGalleryImgListFunc = (id: any) => {
    try {
        ...
        try {
          ...
          await GetDiscardedEggList(params).then(res => {
            if (res?.data?.result?.length > 0) {
              setEggList(prevArray => [...prevArray, ...res?.data?.result])
              setReachedEnd(false)
            } else {
              setReachedEnd(false)
            }
      ...
      >
        ...
        <TabContext value={status}>
          <TabList onChange={handleChange} sx={{ display: 'flex', justifyContent: 'space-between' }}>
            <Tab value='Overview' label={<TabBadge label='Overview' totalCount={0} />} />
            <Tab
              value='Eggs'
              label={<TabBadge label={'Eggs' + (summary?.egg_count > '0' ? ' - ' + eggList?.length : '')} totalCount={eggList?.length || 0} />}
            />
          </TabList>
        ...
        >
          {status === 'Overview' ? (
            ...
            <EggDisCarded
              eggList={eggList}
              getEggListSummary={() => getEggListSummary(eggDiscardedId)}
