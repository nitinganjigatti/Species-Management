...
const EggSecondSecion: any = ({
  ...
  const [rowsWeight, setRowsWeight] = useState<any[]>([])
  ...
  const formatDay = (date: any) => moment(Utility.convertUTCToLocal(date)).format('YYYY-MM-DD')
  ...
  const chartData = sortedRowsForChart.map((row: any) => ({
  ...
  const options = {
    ...
    xaxis: {
      ...
      labels: {
        rotate: -45,
        formatter: (val: any) => moment(val).format('DD MMM') // pretty format
      }
    },
    yaxis: {
      title: { text: 'Weight (g)' }
  ...
  const columns = [
    ...
    },
    {
      ...
      renderCell: (params: any) => (
        <Tooltip
          title={`${Number(params?.row?.assessment_value || 0) % 1 === 0
    ...
    try {
      ...
      await getWeightList(params).then(res => {
        if (res?.success) {
          ...
          let rowWeights = res.data?.result.map((el: any, i: any) => {
            ...
            <CardContent>
              ...
              >
                ...
                <Box>
                  ...
                  >
                    <Grid
                      size={{ xs: 12, sm: 3.55, md: 3.5, lg: 3.55, xl: 3.72, }}
                      sx={{
                    ...
                    <Grid size={{ xs: 12, sm: 3.55, md: 3.5, lg: 3.55, xl: 3.72, }}>
            ...
            <CardContent style={{ paddingBottom: 0 }}>
              ...
              >
                <Table stickyHeader sx={{ borderRadius: '8px' }} aria-label='sticky table'>
                  ...
                  <TableBody>
                    {eggDetails?.assessments_data?.map((row: any, key: any) => {
                      return (
                        <TableRow
                          key={key}
                          sx={{
                            py: 1,
                            '& td': {
                              border: key === 2 ? 'none !important' : undefined
                            }
                          }}
                          hover
                        >
                          ...
                          >
                            <Tooltip
                              title={`${Number(row?.assessment_value || 0) % 1 === 0
