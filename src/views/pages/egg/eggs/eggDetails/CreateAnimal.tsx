...
const CreateAnimalSlider: any = ({ eggId, setOpenDrawer, openDrawer, fetchTableData }: any) => {
  ...
  const getMasterInstitutesFunc = () => {
    try {
      getMasterInstitutes().then((res: any) => {
        if (res.success) {
  ...
  const getMastersOrganizationFunc = () => {
    try {
      getMastersOrganization().then((res: any) => {
        if (res.length) {
  ...
  const getAnimalOwnershipTermsFunc = () => {
    try {
      getAnimalOwnershipTerms().then((res: any) => {
        if (res.success) {
  ...
  const getAnimalGetconfigsFunc = () => {
    try {
      getAnimalGetconfigs().then((res: any) => {
        if (res.success) {
  ...
  const closeEnclosure = () => {
    setEnclosureData({})
    resetField('enclosure_id', { defaultValue: '' })
  }
        ...
        <Box sx={{ bgcolor: theme.palette.customColors.lightBg, width: '100%', height: '100%', overflowY: 'auto' }}>
          ...
          <form onSubmit={handleSubmit(onSubmit)}>
                ...
                <Card sx={{ py: '20px', px: '16px' }}>
                        ...
                        <Autocomplete
                          sx={{
                            '& .MuiOutlinedInput-root': {
                              borderColor: Boolean(errors.species) ? 'red' : 'transparent',
                  ...
                  {Object.keys(enclosureData).length <= 0 && (
                    <div style={{ zIndex: 2, position: 'relative' }}>
                      ...
                      <FormControl fullWidth sx={{ mb: 4 }}>
                        ...
                        />
                        {errors.enclosure_id && (
                          <FormHelperText sx={{ color: 'error.main' }}>{String(errors?.enclosure_id?.message || '')}</FormHelperText>
                        )}
                      </FormControl>
                    </div>
                  )}
                  ...
                  <FormControl fullWidth sx={{ mb: 4 }}>
                    ...
                    {errors?.collectionType && (
                      <FormHelperText sx={{ color: 'error.main' }}>{String(errors?.collectionType?.message || '')}</FormHelperText>
                    )}
                  </FormControl>
                  <FormControl fullWidth sx={{ mb: 4 }}>
                    ...
                    <Controller
                      ...
                      render={({ field: { value, onChange } }) => (
                        ...
                        >
                          {mastersOrganizationList?.map((val: any) => (
                            <MenuItem key={val?.id} value={val?.id}>
                  ...
                  <FormControl fullWidth>
                    ...
                    />
                    {errors.birthDate && (
                      <FormHelperText sx={{ color: 'error.main' }}>{String(errors?.birthDate?.message || '')}</FormHelperText>
                    )}
                  </FormControl>
                  ...
                  <FormControl fullWidth sx={{ mb: 4 }}>
                    ...
                    <Controller
                      ...
                      render={({ field: { value, onChange } }) => (
                        ...
                        >
                          {localIdentifierTypeList?.map((val: any) => (
                            <MenuItem key={val?.id} value={val?.id}>
                              {val?.label}
                            </MenuItem>
                          ))}
                  ...
                  {watch('localIdentifierType') === '' ? null : (
                    <FormControl sx={{ mb: 4 }} fullWidth>
                      ...
                      {errors.localIdentifier && (
                        <FormHelperText sx={{ color: 'error.main' }}>{String(errors?.localIdentifier?.message || '')}</FormHelperText>
                      )}
                    </FormControl>
                  )}
                  <FormControl fullWidth sx={{ mb: 4 }}>
                  ...
                  <FormControl fullWidth>
                    ...
                    <Controller
                      ...
                      render={({ field: { value, onChange } }) => (
                        ...
                        >
                          {contraceptionTypeList?.map((val: any) => (
                            <MenuItem key={val?.id} value={val?.id}>
                              {val?.name}
                            </MenuItem>
                          ))}
                    ...
                    {errors?.contraceptionType && (
                      <FormHelperText sx={{ color: 'error.main' }}>{String(errors?.contraceptionType?.message || '')}</FormHelperText>
                    )}
                  </FormControl>
                </Card>
              </Box>
            ...
            >
              ...
              <LoadingButton disabled={loader} fullWidth variant='contained' loading={loader} type='submit' size='large'>
                {t('submit')}
              </LoadingButton>
            </Box>
          </form>
