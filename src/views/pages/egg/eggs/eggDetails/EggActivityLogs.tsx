...
const EggActivityLogs: any = ({
        ...
        getActivityLogs(egg_id, params).then(res => {
          if (res?.success) {
            if (res?.data?.result?.length > 0) {
              setActivtyLogData((prev: any) => [...prev, ...res?.data?.result])
              setReachedEnd(false)
            } else {
              setReachedEnd(false)
            }
  ...
  const handleToggleComment = (index: any) => {
      ...
      >
        ...
        <Box onScroll={handleScroll} sx={{ px: 4, pt: 8, overflowY: 'auto' }}>
          {activtyLogData?.length > 0 ? (
            <Timeline>
              {activtyLogData?.map((item: any, index: any) => (
                      ...
                      <Box sx={{ flex: 5 }}>
                        <Typography
                          sx={{
                            ...
                            lineHeight: '16.94px',
                            letterSpacing: showCommentIndex === index ? '0.1px' : 'normal',
                            mb: '4px',
                            color:
                              showCommentIndex === index
                                ? theme.palette.primary.contrastText
                                : item.status === 'Necropsy' || item.status === 'Discard' || item.status === 'Rotten'
                                ? theme.palette.formContent?.tertiary || theme.palette.primary.light
                                : theme.palette.primary.light
                          }}
                        >
                          {/* {item.status} */}
