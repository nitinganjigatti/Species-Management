...
import { AllocationSliderProps } from 'src/types/egg/components'

const AllocationSlider: FC<AllocationSliderProps> = ({ setOpenDrawer, allocateEggId, callApi, allocationValues, getDetails }) => {
  ...
  const [assesmentTypes, setAssesmentTypes] = useState<any>({ data: [] })
  ...
  const NurseryList = async (q: string = '') => {
    try {
      const params = {
        page: 1,
        limit: 50,
  ...
  const RoomList = async (id: any, q: string = '') => {
    try {
      const params = {
        page: 1,
        limit: 50,
  ...
  const searchRoom = useCallback(
    debounce(async (id: any, q: string = '') => {
      try {
        await RoomList(id, q)
      } catch (error) {
        console.error(error)
  ...
  const fetchIncubatorData = async (id: any, q: string = '') => {
    const params = {
      room_id: id,
      q,
      type: 'only_active'
  ...
  const fetchData = async () => {
    try {
      ...
      if (assesmentTypes?.data?.length > 0) {
        assesmentTypes.data.forEach((item: any) => {
          append(item)
        })
      }
      setLoader(false)
            ...
            <Box sx={{ px: 4 }}>
              ...
              >
                ...
                <FormControl fullWidth sx={{ width: '95%', ml: 3, mt: 3 }}>
                  ...
                  />
                  {errors?.room && (
                    <FormHelperText sx={{ color: 'error.main' }}>{(errors?.room as any)?.message}</FormHelperText>
                  )}
                </FormControl>
                ...
                <FormControl fullWidth sx={{ width: '95%', ml: 3, mt: 3 }}>
                  ...
                  />
                  {errors?.incubator && (
                    <FormHelperText sx={{ color: 'error.main' }}>{(errors?.incubator as any)?.message}</FormHelperText>
                  )}
                </FormControl>
              </CardContent>
            ...
            {loader ? (
              ...
              >
                <CardContent sx={{ mt: '-1px' }}>
                  {fields.map((measurement: any, index: any) => (
                    <Grid container spacing={3} key={index}>
                      <Grid size={{ xs: 6 }} sx={{ borderRadius: '5px' }}>
                          ...
                          <Controller
                            ...
                            render={({ field: { value, onChange }, fieldState: { error } }) => (
                              <div>
                                <TextField
                                  sx={{ borderRadius: '4px' }}
                                  label={`${measurement?.assessment_type_string_id?.charAt(0)?.toUpperCase() +
                      ...
                      <Grid size={{ xs: 6 }}>
                        <FormControl fullWidth sx={{ mt: 3 }}>
                          ...
                          />
                          {errors && (
                            <FormHelperText sx={{ color: 'error.main' }}>{(errors?.condition as any)?.message}</FormHelperText>
                          )}
                        </FormControl>
                      </Grid>
                      <Grid size={{ xs: 6 }} sx={{ display: 'none' }}>
