...
const EggFilterDrawer = ({
  ...
  useEffect(() => {
    if (filter_list) {
      setFilterList(typeof filter_list === 'string' ? JSON.parse(filter_list) : filter_list)
    }
    if (selected_options) {
      setSelectedOptions(typeof selected_options === 'string' ? JSON.parse(selected_options) : selected_options)
    }
    if (selected_filters_options) {
      setSelectedFiltersOptions(typeof selected_filters_options === 'string' ? JSON.parse(selected_filters_options) : selected_filters_options)
    }
  }, [])
  ...
  const CollectedByList = async (search: any) => {
    try {
      const params = {
        // type: ['length', 'weight'],
        q: search ? search : ''
  ...
  const DiscardedByList = async (search: any) => {
    try {
      const params = {
        type: 'discard',
        q: search ? search : ''
  ...
  useEffect(() => {
    if (selectedDropdownID && eggMaster) {
      const filteredEggStage = eggMaster?.egg_state?.filter((status: any) => status?.egg_status_id === selectedDropdownID)
  ...
  const handleApplyFilter = () => {
    ...
    const statusName = eggMaster?.egg_status?.find((status: any) => status.id === selectedOptions.status)?.egg_status
  ...
  const handleSelectAllChange = (event: any) => {
    ...
    if (isChecked) {
      const newSelectedOptions = {
        ...selectedOptions,
        [selectedMenu.name]: getOptionsForMenu(selectedMenu).map((item: any) => ({ id: item.id, name: item.name }))
      }
  ...
  const handleCheckboxChange = (id: any, name: any) => {
    ...
    const currentSelectedOptions = (selectedOptions as any)[selectedMenu.name] || []
    ...
    switch (menu.name) {
      case 'Stage':
        if (tab_Value === 'eggs_incubation') {
          const filteredEggStage = eggMaster?.egg_state?.filter((stage: any) => stage.egg_status_id === selectedDropdownID)
    ...
    debounce(async (search: any) => {
      ...
      try {
        if (selectedMenu.name === 'Nursery') {
          ...
          const filtered = siteList.filter((site: any) => site.site_name.toLowerCase().includes(search.toLowerCase()))
          if (search) {
          ...
          if (search) {
            const filtered = eggStage.filter((stage: any) => stage.egg_state.toLowerCase().includes(search.toLowerCase()))
            setEggStage(filtered)
          } else {
            if (selectedDropdownID && eggMaster) {
              const filteredEggStatus = eggMaster?.egg_state?.filter(
                (status: any) => status?.egg_status_id === selectedDropdownID
              )
