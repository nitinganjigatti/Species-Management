...
const NecropsySlider: any = ({ eggID, setOpenNecropsy, openNecropsy, fetchTableData }: { eggID: any, setOpenNecropsy: any, openNecropsy: any, fetchTableData: any }) => {
  ...
  const schema = yup.object().shape({
    report_file: (yup.mixed() as any).when('isSampleTaken', {
      is: false,
                ...
                <Grid container sx={{ justifyContent: 'space-between' }}>
                  {imgSrc !== '' ? null : (
                    <Grid size={{ xs: 12, sm: 12, md: 12, }}>
                      ...
                      {errors?.report_file && (
                        <FormHelperText sx={{ color: 'error.main' }}>{String(errors?.report_file?.message)}</FormHelperText>
                      )}
                    </Grid>
                  )}
                  <Grid size={{ md: 12 }} >
