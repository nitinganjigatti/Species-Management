...
const EggTableHeader: FC<any> = ({
  ...
  const handleExport = async () => {
    try {
      ...
      const discardedByIds = selectedFiltersOptions['Discarded By']?.map((option: any) => option.id) || []
      const activeStatus = selectedFiltersOptions['Security Check']?.map((option: any) => option.id) || []
      ...
      await apiToUse({ params: params }).then(res => {
        if (res.success || res.data.success) {
          ...
          const tableData = ListData.map((item: any, index: any) => {
        ...
        <Box>
          {eggViewInsights && (
            <Typography sx={{ fontSize: '14px', fontWeight: 300 }}>
              ...
              : <span style={{ fontWeight: 500, color: (theme.palette.primary as any).deepDark }}>{totalCount}</span>
            </Typography>
          )}
        </Box>
          ...
          >
            ...
            <TextField
              ...
              onChange={e => {
                ...
                router.push({ pathname: router.pathname, query: { ...router.query, search_value: e.target.value } }, undefined, { shallow: true })
              }}
              sx={{
