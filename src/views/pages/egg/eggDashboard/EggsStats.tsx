...
const StatBox: FC<StatBoxProps> = ({ avatarSrc, value, label, tooltipContent, onClick, valueColor }) => {
...
interface EggsStatsProps {
  allStats: any
  setOpenDiscard: (value: boolean) => void
}

const EggsStats: FC<EggsStatsProps> = ({ allStats, setOpenDiscard }) => {
  const theme = useTheme()
  const { t } = useTranslation()
  ...
  const statsData = [
    ...
    },
    {
      ...
      valueColor: theme.palette.formContent?.tertiary || theme.palette.primary.main
    },
    {
  ...
  return (
    <Grid container spacing={3} columns={5}>
      {statsData.map((stat: any, index: any) => (
        <Grid size={{ xs: 5, sm: 2.5, lg: 1 }} key={index}>
          <StatBox {...stat} />
        </Grid>
      ))}
    </Grid>
