...
interface TodaysCollectionData {
  total_eggs?: number
  total_eggs_in_nest?: number
  total_eggs_in_nursery?: number
  total_hatched?: number
  total_discarded?: number
}

const TodaysCollection: FC = () => {
  const authData = useContext(AuthContext)
  const { t } = useTranslation()
  const theme = useTheme()
  const [todaysCollection, setTodaysCollection] = useState<TodaysCollectionData | null>(null)
  ...
  const getTodaysCollectionFunc = (): void => {
    try {
      getTodaysCollection().then((res: any) => {
        if (res?.data?.success) {
      ...
      Toaster({ type: 'error', message: error as string })
    }
  }
                  ...
                  >
                    ...
                    <Typography
                      sx={{
                        ...
                        fontSize: '20px',
                        lineHeight: '24.2px',
                        color: theme.palette.formContent?.tertiary || theme.palette.primary.main
                      }}
                    >
                      {todaysCollection?.total_discarded}
                    </Typography>
