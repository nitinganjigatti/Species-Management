...
interface Props {
  ...
  allComboSelectedValues: any[]
  formData: any
  addEventSidebarOpen: any
  searchValue: string
  setSearchValue: (...args: any[]) => void
  fromrow: any
  comboid: any
  comboName: any
  cutsizelist: any[]
...
const ComboCard = ({
  ...
  useEffect(() => {
    const uniqueSelectedValues = allComboSelectedValues?.filter((value: any, index: any, self: any) =>
        index === self.findIndex((v: any) => v?.recipe_id === value?.recipe_id && v?.mealid === value?.mealid)
    )

    const selectedValuesWithCheckId = uniqueSelectedValues?.filter(item => item?.mealid === checkid)

    let updatedselectedCardCombo: any[] = []
    ...
    rows.forEach(row => {
      ...
      if (match) {
        ...
        for (const key in row) {
          updatedRow[key as string] = match[key] !== undefined ? match[key] : row[key]
        }
    ...
    if (
      ...
      selectedValuesWithCheckId?.forEach(item => {
        if (item.mealid === checkid) {
          updatedRemarks[item.recipe_id as string] = item.remarks || ''
        }
      })
  ...
  const handleAddRemarks = (event: any, cardId: any) => {
  ...
  const handleChangeSize = (event: any, item: any, ingredient: any, idx: any) => {
