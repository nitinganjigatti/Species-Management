...
interface Props {
  openDrawer: any
  handleCloseSideBar: (...args: any[]) => any
  selectedItems: any[]
  ...
  loader: any
  mealId: any
  handleEditSearch: (...args: any[]) => any
  fetchSiteStats: (...args: any[]) => any
}

const CreateMealGroup: React.FC<Props> = ({
  openDrawer,
  handleCloseSideBar,
  selectedItems,
  setSelectedItems,
  ...
  const [groupName, setGroupName] = useState<string>(editParam?.group_name || '')
  const [groupNameError, setGroupNameError] = useState<boolean>(false)
  const [searchTerm, setSearchTerm] = useState<string>('')
  const [removedEnclosures, setRemovedEnclosures] = useState<any[]>([])
  const [loading, setLoading] = useState<boolean>(false)
  const toastLock = useRef<any>(null)
  ...
  const handleEnclosureRemove = async (index: number) => {
