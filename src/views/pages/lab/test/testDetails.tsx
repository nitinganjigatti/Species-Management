...
import type { TestDetailsProps } from 'src/types/lab'
...
interface TestDetailData {
  id?: number
  label?: string
  sample_types?: SampleTypeItem[]
  child_tests?: ChildTestItem[]
...
const TestDetails = (props: TestDetailsProps) => {
  ...
  const getLabTestById = useCallback(async (id: number | string) => {
    ...
    if (response?.success) {
      setTestDetails(response.data as TestDetailData)
      setLoading(false)
    } else {
      setLoading(false)
    }
  ...
  const handleDelete = () => {
    ...
    setSelectedId(testDetails?.id ?? null)
  }
                ...
                {testDetails && (
                  <Box>
                    <CardContent>
                      ...
                      {(testDetails.child_tests?.length ?? 0) > 0 && (
                        <Typography
                          variant='subtitle2'
                          sx={{
                            color: 'text.secondary',
