...
interface SingleSelectSectionListProps {
  open: boolean
  onClose?: () => void
  siteId?: string | number
  onSelectSections: (sections: (string | number)[]) => void
  setSectionsData: (data: SectionRecord[] | ((prev: SectionRecord[]) => SectionRecord[])) => void
  sectionsData: SectionRecord[]
  setSelectedSections?: (sections: (string | number)[]) => void
...
const SingleSelectSectionList = ({
  ...
  const handleSectionRadioChange = (sectionId: string | number) => {
  ...
  const fetchSections = async (searchQuery: string = searchTerm) => {
    if (!siteId) return
    setLoading(true)
    try {
      const params = {
      ...
      const newSections: SectionRecord[] = response.data?.result || []
