...
interface ObservationData {
  child_enrichment_type: string | string[]
  master_enrichment_type: string
  date_time: string
}
...
const ObservationView = ({ data: { child_enrichment_type, master_enrichment_type, date_time } }: ObservationViewProps) => {
  const theme = useTheme()

  // Parse child enrichment types from comma-separated string
  const childEnrichmentTypes: string[] = Array.isArray(child_enrichment_type)
  ...
  const formatDateTime = (dateTime: string) => {
    ...
    const [date = '', time = ''] = formattedDateStr.split('|').map((part: string) => part.trim())
