...
interface AddEditDocumentTypeProps {
  open: boolean
  handleClose: () => void
  handleSubmitData: (payload: DocumentTypeFormValues, editId: Id | null) => void | Promise<void>
  submitLoader?: boolean
  tradeContextTypes?: TradeContextType[]
  contextLoading?: boolean
  editId?: Id | null
  initialValues?: DocumentTypeFormValues | null
}

const defaultValues: DocumentTypeFormValues = {
  name: '',
  description: '',
  contexts: [],
  active: '1'
...
const AddEditDocumentType = ({
  ...
  const onSubmit = async (data: DocumentTypeFormValues) => {
    const payload = {
      ...data,
      contexts: (data.contexts as unknown[]).map(Number) as Id[]
    }
