...
interface AnimalEntry {
  ...
  species?: string
  microchip?: string
  microchipValue?: string
  id?: string | number
}

interface ExportSpecies {
  common_name?: string
  animals?: Array<{
    gender?: string
...
interface OtherItem {
  species?: ExportSpecies | ExportSpecies[]
  [key: string]: unknown
}
...
interface AnimalIdentifiersProps {
  selectedExportData: SelectedExportData
}

const AnimalIdentifiers = ({ selectedExportData }: AnimalIdentifiersProps) => {
  ...
  const animals: AnimalEntry[] = [
    ...
    ...(selectedExportData?.others?.flatMap(otherItem => {
      const speciesArray = Array.isArray(otherItem?.species) ? otherItem.species : [otherItem?.species].filter(Boolean) as ExportSpecies[]
