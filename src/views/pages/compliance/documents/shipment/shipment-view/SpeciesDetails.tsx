...
import { Box, Typography, Paper, Chip, Collapse, Divider, alpha } from '@mui/material'
...
interface SpeciesDetailsContainerProps {
  totalSpecies: number
...
const SpeciesDetailsContainer = ({
  ...
  const OthersSection = ({ data, isCollapsed }: OthersSectionProps) => {
  ...
  const ExportSection = ({ data, isCollapsed, isLast }: ExportSectionProps) => {
    ...
    return (
      <>
        <Box>
          ...
          <Collapse in={!isCollapsed}>
            <Paper
              ...
              sx={{
                borderRadius: 0,
                borderBottomLeftRadius: isLast && (selectedExportData?.others?.length ?? 0) <= 0 ? '8px' : '0px',
                borderBottomRightRadius: isLast && (selectedExportData?.others?.length ?? 0) <= 0 ? '8px' : '0px'
              }}
            >
        ...
        {!isLast || (selectedExportData?.others?.length ?? 0) > 0 ? <Divider /> : ''}
      </>
    )
  }
      ...
      >
        ...
        {selectedExportData?.export?.map((exp, idx) => (
          <ExportSection
            ...
            isLast={idx === (selectedExportData?.export?.length ?? 0) - 1}
          />
        ))}

        {(selectedExportData?.others?.length ?? 0) > 0 && (
