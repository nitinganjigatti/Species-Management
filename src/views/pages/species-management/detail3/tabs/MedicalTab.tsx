...
import { useSortableTable } from 'src/views/pages/species-management/detail3/useSortableTable'
                ...
                {history.map((e, i) => (
                  <SheetRow
                    ...
                    trailing={
                      e.outcome === 'died' ? (
                        <>
                          <MedTagPill label='Died' />
                          <Typography sx={{ fontSize: '12px', color: c.Outline }}>{fmtDate(e.date)}</Typography>
                        </>
                      ) : (
                        <>
                          <Typography sx={{ fontSize: '12px', color: c.Outline }}>
                            {fmtDate(e.date)} → {fmtDate(addDays(e.date, e.durationDays ?? 0))}
                          </Typography>
                          <Typography sx={{ fontSize: '12px', color: c.neutralSecondary }}>{e.durationDays ?? 0} d</Typography>
                        </>
                      )
                    }
                  />
