...
import { SectionCard, useTone, thinScrollbarSx } from 'src/views/pages/species-management/ipad1/detail/detailUi'
...
const baseChartOptions = (theme: any) => ({
  ...
  tooltip: { theme: theme.palette.mode, style: { fontSize: '16px' } },
...
const tooltipHTML = (theme: any, title: string, rows: { color: string; label: string; value: string }[]) => {
  ...
  return `<div style="font-size:16px;font-family:inherit;background:${theme.palette.background.paper};">${head}${body}</div>`
}
    ...
    {data
      ...
      .map((d, i) => (
        ...
        >
          ...
          <Typography variant='caption' sx={{ fontSize: '16px', color: 'customColors.OnSurfaceVariant' }}>
            {d.label}{' '}
...
export const RankedBarChart: React.FC<{ segments: CompositionSegment[]; horizontal: boolean; height?: number; barHeight?: string }> = ({
  ...
  const options = {
    ...
    legend: { show: false },
    grid: { ...baseChartOptions(theme).grid, padding: { top: -20 } },
    xaxis: {
      categories: data.map(d => d.label),
      labels: { style: { colors: cc.neutralSecondary, fontSize: '16px' } },
      axisBorder: { show: false },
      axisTicks: { show: false }
    },
    yaxis: { labels: { style: { colors: cc.neutralSecondary, fontSize: '16px' }, maxWidth: 220 } },
    tooltip: facetTooltip(theme, data, colors)
  }
  ...
  // Vertical mode — scroll standard: readable slot per category, thin visible scrollbar.
  return (
    <Box sx={{ position: 'relative', height: height + 12 }}>
      <Box sx={{ position: 'absolute', inset: 0, overflowX: 'auto', overflowY: 'hidden', ...thinScrollbarSx(theme) }}>
        <Box sx={{ minWidth: `${data.length * 72}px` }}>
          <ReactApexcharts
            type='bar'
            height={height}
            options={options}
  ...
  const options = {
    ...
    plotOptions: {
      radialBar: {
        ...
        dataLabels: {
          name: { fontSize: '14px', color: cc.neutralSecondary },
...
export function ColumnBarChart({
  ...
  const buildOptions = (axisOnly: boolean): any => ({
    ...
    dataLabels:
      showValues && !axisOnly
        ? {
            ...
            style: { fontSize: '14px', fontWeight: 700, colors: [color] }
          }
        : { enabled: false },
    legend: { show: false },
    grid: {
    ...
    xaxis: {
      ...
      labels: { style: { colors: axisOnly ? 'transparent' : cc.neutralSecondary, fontSize: '14px' } },
      axisBorder: { show: false },
      axisTicks: { show: false }
    },
    yaxis:
      hideYAxis || !axisOnly
        ...
        : {
            ...
            min: 0,
            forceNiceScale: true,
            labels: { style: { colors: cc.neutralSecondary, fontSize: '14px' }, align: 'left', minWidth: 24, offsetX: -14 }
          },
    tooltip: axisOnly
      ? { enabled: false }
      : {
  ...
  return (
    <ReactApexcharts
      ...
      options={{
        ...
        xaxis: {
          categories: labels,
          labels: { style: { colors: cc.neutralSecondary, fontSize: '14px' }, hideOverlappingLabels: true, rotate: 0 },
          axisBorder: { show: false },
          axisTicks: { show: false },
        ...
        yaxis: {
          tickAmount: 4,
          labels: { style: { colors: cc.neutralSecondary, fontSize: '14px' }, formatter: (v: number) => Math.round(v).toLocaleString() }
        },
        tooltip: {
          custom: ({ series, seriesIndex, dataPointIndex }: any) =>
            tooltipHTML(theme, labels[dataPointIndex] ?? '', [
