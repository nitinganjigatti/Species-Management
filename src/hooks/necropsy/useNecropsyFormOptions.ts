...
import type { SelectOption, WeightUnitOption, UseNecropsyFormOptionsReturn } from 'src/types/necropsy'
 ...
 */
export const useNecropsyFormOptions = (autoFetch: boolean = true): UseNecropsyFormOptionsReturn => {
  ...
  const findMannerOfDeathOption = useCallback(
    (idOrName: string | number | null | undefined): SelectOption | null => {
  ...
  const findWeightUnitOption = useCallback(
    (idOrAbbr: string | number | null | undefined): WeightUnitOption | null => {
      ...
      if (!option) {
        option = weightUnitOptions.find(
          (u) =>
            abbrLower.includes(u.value?.toLowerCase()) || u.value?.toLowerCase()?.includes(abbrLower)
        )
      }
