'use client';
import { useState, useEffect, useContext, useCallback } from 'react'
import { Card, CardContent, Divider, Breadcrumbs, Link, debounce, Box, Typography } from '@mui/material'
import Icon from 'src/@core/components/icon'
import Step from '@mui/material/Step'
import Stepper from '@mui/material/Stepper'
import StepLabel from '@mui/material/StepLabel'
import StepBasicDetails from 'src/views/pages/diet/add-diet/StepBasicDetails'
import { getIngredientList } from 'src/lib/api/diet/getIngredients'
import Toaster from 'src/components/Toaster'
import StepperCustomDot from 'src/views/forms/form-wizard/StepperCustomDot'
import StepperWrapper from 'src/@core/styles/mui/stepper'
import { addNewDiet, getDietDetails, updateDiet } from 'src/lib/api/diet/dietList'
import dayjs from 'dayjs'
import useSafeRouter from 'src/hooks/useSafeRouter';
import { useParams, useSearchParams } from 'next/navigation';
import StepPreviewDiet from 'src/views/pages/diet/add-diet/PreviewDiet'
import { getDietTypeList } from 'src/lib/api/diet/dietList'
import { AuthContext } from 'src/context/AuthContext'
import { getCutsizeList } from 'src/lib/api/diet/settings/cutSizes'
import { getFeedTypeList } from 'src/lib/api/diet/feedType'
import { getUnitsForRecipe } from 'src/lib/api/diet/recipe'
import Error404 from 'src/pages/404'
import { readAsync } from 'src/lib/windows/utils'
import { getUserList } from 'src/lib/api/pharmacy/dispenseProduct'

const steps = [
  {
    title: 'Basic Information',
    subtitle: 'Enter details'
  },
  {
    title: 'Add Quantity',
    subtitle: 'Enter details'
  }
]

const AddDiet = () => {
  const router = useSafeRouter();
  const params = useParams();
  const searchParams = useSearchParams();
  const routerQuery = { ...params, ...(searchParams ? Object.fromEntries(searchParams.entries()) : {}) };
  const { id, name } = routerQuery
  const [activeStep, setActiveStep] = useState(0)
  const [uomList, setUomList] = useState<any[]>([])
  const [dieticianList, setDieticianList] = useState<any[]>([])
  const [uomprevnew, setUomprevnew] = useState<any[]>([])
  const [IngredientTypeList, setIngredientTypeList] = useState<any[]>([])
  const [selectedCardRecipe, setSelectedCardRecipe] = useState<any[]>([])
  const [selectedCardCombo, setSelectedCardCombo] = useState<any[]>([])
  const [diettypechildvalues, setdiettypechildvalues] = useState<any[]>([])
  const [urlType, seturlType] = useState('')
  const [loader, setLoader] = useState(false)
  const [cutsizelist, setcutsize] = useState<any[]>([])
  const [uom, setUom] = useState<any[]>([])
  const [feedType, setFeedType] = useState<any[]>([])
  const [feedtotalCount, setFeedTotalCount] = useState(0)
  const [page, setPage] = useState(1)
  const [loadingfeed, setLoadingFeed] = useState(false)

  const authData = useContext(AuthContext) as any
  const dietModule = authData?.userData?.roles?.settings?.diet_module
  const dietModuleAccess = authData?.userData?.roles?.settings?.diet_module_access

  const [formData, setFormData] = useState<any>({
    diet_name: '',
    diet_type_name: '',
    diet_type_id: '',
    dietitian_name: '',
    dietitian_id: null,
    child: '',
    diet_image: '',
    desc: '',
    remarks: '',
    meal_data: [
      {
        mealid: 'meal0',
        meal_name: 'Meal 1',
        meal_from_time: '',
        meal_to_time: '',
        notes: '',
        recipe: [],
        combo: [],
        ingredient: [],
        ingredientwithchoice: []
      }
    ]
  })

  const handleSelectedCardChange = (card: any) => {
    setSelectedCardRecipe(card)
  }

  const handleSelectedCardChangeCombo = (card: any) => {
    setSelectedCardCombo(card)
  }

  const getUnitsList = async () => {
    try {
      const params = {
        // type: ['length', 'weight'],
        // page: 1
      }
      await getDietTypeList({ params: params }).then(res => {
        setUomList(res?.data)
      })
    } catch (e: any) {
      console.log(e)
    }
  }

  const getUnitsListuom = async () => {
    try {
      const params = {
        type: ['length', 'weight'],
        page: 1,
        limit: 150
      }
      await getUnitsForRecipe({ params: params }).then(res => {
        setUomprevnew(res?.data?.result)
      })
    } catch (e: any) {
      console.log(e)
    }
  }

  const getcutsizeListFromApi = async () => {
    try {
      const params = {
        //type: ['length', 'weight'],
        page: 1,
        limit: 200
      }
      await getCutsizeList(params).then(res => {
        setcutsize(res?.data?.result)
        setUom(res?.data?.result)
      })
    } catch (e: any) {
      console.log(e)
    }
  }

  const getDieticianList = async () => {
    try {
      const userDetails = await readAsync('userDetails')
      if (userDetails?.user?.zoos.length > 0) {
        let zoo_id = userDetails?.user?.zoos[0].zoo_id
        await getUserList({ zoo_id }).then(res => {
          if (res?.data?.length > 0) {
            setDieticianList(
              res?.data?.map((item: any) => ({
                label: item?.user_name,
                value: item?.user_id
              }))
            )
          }
        })
      }
    } catch (error: any) {
      console.log('user error', error)
    }
  }

  const [feedSearchValue, setFeedSearchValue] = useState('')

  const fetchData = useCallback(
    async (pageNum = 1) => {
      setLoadingFeed(true)
      const params = { page: pageNum, limit: 20, status: 1, q: feedSearchValue }

      try {
        const response = await getFeedTypeList(params)
        const newData = response?.data?.result || []
        const total = response?.data?.total_count || 0

        setFeedTotalCount(total)
        setFeedType(prev => (pageNum === 1 ? newData : [...prev, ...newData]))
      } catch (error: any) {
        console.log('error', error)
      } finally {
        setLoadingFeed(false)
      }
    },
    [feedSearchValue]
  )

  const handleFeedSearch = useCallback(
    debounce((val: any) => {
      setFeedSearchValue(val)
      setPage(1)
    }, 500),
    []
  )

  const callIngredientTypeList = async ({ status, page, limit, q }: { status?: any; page?: any; limit?: any; q?: any }) => {
    try {
      const params = {
        //status,
        q,

        //active: 1,
        page,
        limit
      }
      await getIngredientList({ params: params }).then(res => {
        setIngredientTypeList(res?.data?.result)
      })
    } catch (e: any) {
      console.log(e)
    }
  }

  const IngredientTypeListSearch = debounce(async (value: any) => {
    try {
      await callIngredientTypeList({ status: 1, page: 1, limit: 20, q: value })
    } catch (e: any) {
      console.log(e)
    }
  }, 500)

  const handleRemarksChange = (newRemarks: any) => {
    setFormData((prevFormData: any) => ({
      ...prevFormData,
      remarks: newRemarks
    }))
  }

  // const handleDietTypeChildValuesChange = values => {
  //   // Update the parent component state with the received values
  //   setdiettypechildvalues(values)
  // }

  // const callIngredientTypeList = async ({ status, page, limit, q }) => {
  //   try {
  //     const params = {
  //       //status,
  //       q,

  //       //active: 1,
  //       page,
  //       limit
  //     }
  //     await getIngredientList({ params: params }).then(res => {
  //       setIngredientTypeList(res?.data?.result)
  //     })
  //   } catch (e) {
  //     console.log(e)
  //   }
  // }

  useEffect(() => {
    if (id) {
      const url = new URL(window.location.href)
      const action = url.searchParams.get('action')
      seturlType(action || '')
    }
  }, [id])

  const handleCancelIconClick = async () => {
    setFormData((prevData: any) => ({
      ...prevData,
      by_quantity: prevData.by_quantity.map((item: any) => ({
        ...item,
        ingredient_id: '',
        ingredient_name: '',
        feed_type_label: ''
      }))
    }))
    callIngredientTypeList({ status: 1, page: 1, limit: 10, q: '' })
  }

  useEffect(() => {
    if (id && urlType && (dietModuleAccess === 'ADD' || dietModuleAccess === 'EDIT' || dietModuleAccess === 'DELETE')) {
      getIngredientsDetailval(id)
    }
  }, [id, urlType])

  const getIngredientsDetailval = async (id: any) => {
    try {
      setLoader(true)
      const response = await getDietDetails(id, { week_day: 0 })
      console.log(response, 'response')
      if (response.success === true && response.data !== null) {
        const data = response.data

        setFormData((prevFormData: any) => ({
          ...prevFormData,
          diet_name: urlType === 'copy' ? name : data.diet_name,
          diet_type_name: data.diet_type_name,
          diet_type_id: data.diet_type_id,
          dietitian_id: data?.dietitian_id,
          dietitian_name: data?.dietitian?.user_name,
          child: data.child,
          diet_image: data.diet_image,
          desc: data.desc,
          remarks: data.remarks,
          meal_data: data.meal_data.map((meal: any) => {
            const parseTime = (time: any) => {
              if (time.toLowerCase().includes('am') || time.toLowerCase().includes('pm')) {
                return dayjs(time, 'hh:mm A')
              }

              return dayjs(time, 'HH:mm')
            }

            return {
              ...meal,
              meal_from_time: parseTime(meal.meal_from_time),
              meal_to_time: parseTime(meal.meal_to_time)
            }
          })
        }))

        const dietTypesData = data.child

        const convertedData = dietTypesData?.map((item: any) => item.replace(/(\d+) /g, '$1_'))

        const newarr = convertedData?.map((item: any) => {
          const [weight, unitName] = item.split('_')
          const matchedUom = uomprevnew.find((u: any) => u.name === unitName)

          return {
            meal_value_header: parseFloat(weight),
            weight_uom_id: parseFloat(matchedUom?._id),
            weight_uom_label: unitName
          }
        })

        const _newarrdiet = newarr?.map((item: any, _index: any) => ({
          unit: {
            value: {
              _id: item.weight_uom_id,
              name: item.weight_uom_label,
              description: item.weight_uom_label
            }
          },
          weight: parseInt(item.meal_value_header)
        }))

        document.cookie = `dietTypeChildValues=${JSON.stringify(data.child)}; path=/`
        document.cookie = `dietTypeChildVal=${JSON.stringify(newarr)}; path=/`
        setLoader(false)
      }
    } catch (error: any) {
      console.log('Feed list', error)
    }
  }

  // Function to format time
  // const formatTime = timeString => {
  //   const date = new Date(`2000-01-01 ${timeString}`)

  //   return date.toUTCString()
  // }

  const formatTime = (timeString: any) => {
    return dayjs(timeString).format('HH:mm') // Standardize the format
  }

  useEffect(() => {
    fetchData(page)
  }, [page, fetchData])

  useEffect(() => {
    getUnitsList()
    getcutsizeListFromApi()
    getUnitsListuom()
    getDieticianList()
  }, [])

  const handleNext = (data: any) => {
    // setFormData(prevData => ({
    //   ...prevData,
    //   ...newData
    // }))
    setFormData({ ...formData, ...data })
    setActiveStep(activeStep + 1)
  }

  const handlePrev = () => {
    if (activeStep !== 0) {
      setActiveStep(activeStep - 1)
    }
  }

  function deleteCookie(name: any) {
    document.cookie = name + '=; expires=Thu, 01 Jan 1970 00:00:00 UTC; path=/;'
  }

  const updateFormData = (newData: any) => {
    setFormData((prevData: any) => ({
      ...prevData,
      ...newData
    }))
  }

  useEffect(() => {
    deleteCookie('dietTypeChildValues')
    deleteCookie('dietTypeChildVal')
  }, [])

  const handleLoadMore = () => {
    if (!loadingfeed && feedType.length < feedtotalCount) {
      setPage(prev => prev + 1)
    }
  }

  const handleStepBillingSubmit = async () => {
    console.log(formData, 'formdata')

    const deepCleanImages = (data: any): any => {
      if (Array.isArray(data)) {
        return data.map((item: any) => deepCleanImages(item))
      } else if (data !== null && typeof data === 'object') {
        const cleaned: Record<string, any> = {}
        for (const [key, value] of Object.entries(data)) {
          if (key !== 'recipe_image' && key !== 'ingredient_image') {
            cleaned[key] = deepCleanImages(value)
          }
        }
        return cleaned
      }
      return data
    }

    let mealTypeError = false
    let genericError = false

    formData.meal_data.forEach((item: any) => {
      const { ingredientwithchoice, recipe, combo, ingredient } = item

      const checkMealType = (data: any) => {
        if (data && data.length > 0) {
          if (!data.every((d: any) => d.meal_type && Array.isArray(d.meal_type) && d.meal_type.length > 0)) {
            mealTypeError = true

            return false
          }
          if (!data.every((d: any) => d.meal_type.some((mealType: any) => mealType.meal_value_header === 'Generic'))) {
            genericError = true

            return false
          }
        }

        return true
      }

      checkMealType(ingredientwithchoice)
      checkMealType(recipe)
      checkMealType(combo)
      checkMealType(ingredient)
    })

    if (mealTypeError) {
      return Toaster({ type: 'error', message: 'Enter the values of the meal' })
    }

    if (genericError) {
      return Toaster({ type: 'error', message: 'Please enter all the Generic values' })
    }

    if (!id) {
      // Omitting child field from formData
      // const { child, ...formDataWithoutChild } = formData
      setLoader(true)

      const numericFormData: any = {
        // ...formDataWithoutChild,
        ...formData,
        child: JSON.stringify(formData.child),
        meal_data: JSON.stringify(
          formData.meal_data.map((item: any) => {
            const fromTime = formatTime(item.meal_from_time)
            const toTime = formatTime(item.meal_to_time)

            const filteredItem = Object.fromEntries(
              Object.entries(item).filter(([key, value]) => {
                return !Array.isArray(value) || (value as any[]).some((val: any) => val !== null && val !== undefined)
              }).map(([key, value]) => {
                if (Array.isArray(value)) {
                  return [key, deepCleanImages(value)]
                }
                return [key, value]
              })
            )

            return {
              ...filteredItem,
              mealid: item.mealid,
              meal_name: item.meal_name,
              meal_from_time: fromTime,
              meal_to_time: toTime,
              notes: item.notes

              // recipe: item?.recipe,
              // ingredient: item?.ingredient,
              // ingredientwithchoice: item?.ingredientwithchoice
            }
          })
        )
      }

      const updatedFormData: any = {
        ...numericFormData,
        meal_data: numericFormData.meal_data,
        diet_image: numericFormData?.diet_image?.length > 0 ? numericFormData.diet_image[0] : null
      }

      const apival = await addNewDiet(updatedFormData)
      if (apival.success === true) {
        router.push(`/diet/diet`)
        deleteCookie('dietTypeChildValues')
        deleteCookie('dietTypeChildVal')
        setLoader(false)

        return Toaster({ type: 'success', message: apival.message })
      } else {
        setLoader(false)

        return Toaster({
          type: 'error',
          message: apival?.message?.diet_image ? 'Image type only PNG and JPG is allowed' : apival?.message
        })
      }
    } else if (id && urlType === 'copy') {
      setLoader(true)

      const numericFormData: any = {
        //...formDataWithoutChild,
        ...formData,
        child: JSON.stringify(formData.child),
        meal_data: JSON.stringify(
          formData.meal_data.map((item: any) => {
            const fromTime = formatTime(item.meal_from_time)
            const toTime = formatTime(item.meal_to_time)

            const filteredItem = Object.fromEntries(
              Object.entries(item).filter(([key, value]) => {
                return !Array.isArray(value) || (value as any[]).some((val: any) => val !== null && val !== undefined)
              }).map(([key, value]) => {
                if (Array.isArray(value)) {
                  return [key, deepCleanImages(value)]
                }
                return [key, value]
              })
            )

            return {
              ...filteredItem,
              mealid: item.mealid,
              meal_name: item.meal_name,
              meal_from_time: fromTime,
              meal_to_time: toTime,
              notes: item.notes

              // recipe: item?.recipe,
              // ingredient: item?.ingredient,
              // ingredientwithchoice: item?.ingredientwithchoice
            }
          })
        )
      }

      const updatedFormData: any = {
        ...numericFormData,
        meal_data: numericFormData.meal_data
      }
      if (formData.diet_image === null) {
        delete updatedFormData.diet_image
        delete updatedFormData.remove_current_image
      } else if (typeof formData.diet_image === 'string') {
        delete updatedFormData.diet_image
        delete updatedFormData.remove_current_image
      } else {
        updatedFormData.diet_image = formData.diet_image?.[0] || null
        updatedFormData.remove_current_image = '1'
      }

      const apival = await addNewDiet(updatedFormData)

      if (apival.success === true) {
        router.push(`/diet/diet`)
        deleteCookie('dietTypeChildValues')
        deleteCookie('dietTypeChildVal')
        setLoader(false)

        return Toaster({ type: 'success', message: apival.message })
      } else {
        setLoader(false)

        return Toaster({
          type: 'error',
          message: apival?.message?.diet_image ? 'Image type only PNG and JPG is allowed' : apival?.message
        })
      }
    } else {
      setLoader(true)

      const numericFormData: any = {
        //...formDataWithoutChild,
        ...formData,
        child: JSON.stringify(formData.child),
        meal_data: JSON.stringify(
          formData.meal_data.map((item: any) => {
            const fromTime = formatTime(item.meal_from_time)
            const toTime = formatTime(item.meal_to_time)

            const filteredItem = Object.fromEntries(
              Object.entries(item).filter(([key, value]) => {
                return !Array.isArray(value) || (value as any[]).some((val: any) => val !== null && val !== undefined)
              }).map(([key, value]) => {
                if (Array.isArray(value)) {
                  return [key, deepCleanImages(value)]
                }
                return [key, value]
              })
            )

            return {
              ...filteredItem,
              mealid: item.mealid,
              meal_name: item.meal_name,
              meal_from_time: fromTime,
              meal_to_time: toTime,
              notes: item.notes

              // recipe: item?.recipe,
              // ingredient: item?.ingredient,
              // ingredientwithchoice: item?.ingredientwithchoice
            }
          })
        )
      }

      const updatedFormData: any = {
        ...numericFormData,
        meal_data: numericFormData.meal_data
      }

      if (formData.diet_image === null) {
        delete updatedFormData.diet_image
        delete updatedFormData.remove_current_image
      } else if (typeof formData.diet_image === 'string') {
        delete updatedFormData.diet_image
        delete updatedFormData.remove_current_image
      } else {
        updatedFormData.diet_image = formData.diet_image?.[0] || null
        updatedFormData.remove_current_image = '1'
      }

      const apival = await updateDiet(id as string, updatedFormData)

      if (apival.success === true) {
        router.push(`/diet/diet`)
        deleteCookie('dietTypeChildValues')
        deleteCookie('dietTypeChildVal')
        setLoader(false)

        return Toaster({ type: 'success', message: apival.message })
      } else {
        setLoader(false)

        return Toaster({
          type: 'error',
          message: apival?.message?.diet_image ? 'Image type only PNG and JPG is allowed' : apival?.message
        })
      }
    }
  }

  const getStepContent = (step: any) => {
    switch (step) {
      case 0: {
        const StepBasicDetailsComp = StepBasicDetails as any
        return (
          <StepBasicDetailsComp
            handleNext={handleNext}
            formData={formData}
            updateFormData={updateFormData}
            uomList={uomList}
            dieticianList={dieticianList}
            setSelectedCardRecipe={handleSelectedCardChange}
            setSelectedCardCombo={handleSelectedCardChangeCombo}
            selectedCardRecipe={selectedCardRecipe}
            selectedCardCombo={selectedCardCombo}
            setFormData={setFormData}
            setUomprevnew={setUomprevnew}
            diettypechildvalues={diettypechildvalues}
            id={id}
            loader={loader}
            cutsizelist={cutsizelist}
            uom={uom}
            feedType={feedType}
            onLoadMore={handleLoadMore}
            loadingfeed={loadingfeed}
            feedtotalCount={feedtotalCount}
            handleFeedSearch={handleFeedSearch}
          />
        )
      }
      case 1: {
        const StepPreviewDietComp = StepPreviewDiet as any
        return (
          <StepPreviewDietComp
            handleNext={handleNext}
            handlePrev={handlePrev}
            updateFormData={updateFormData}
            formData={formData}
            uomList={uomList}
            dieticianList={dieticianList}
            IngredientTypeList={IngredientTypeList}
            IngredientTypeListSearch={IngredientTypeListSearch}
            onCancelIconClick={handleCancelIconClick}
            finalhandleSubmit={handleStepBillingSubmit}
            uomprevnew={uomprevnew}
            setFormData={setFormData}
            id={id}
            remarks={formData.remarks}
            onRemarksChange={handleRemarksChange}
            loader={loader}

            // onDietTypeChildValuesChange={handleDietTypeChildValuesChange}
            // diettypechildvalues={diettypechildvalues}
          />
        )
      }
      default:
        return null
    }
  }

  const renderContent = () => {
    return getStepContent(activeStep)
  }

  const contentRenderFunction = () => {
    return (
      <>
        <Breadcrumbs aria-label='breadcrumb' sx={{ mb: 5 }}>
          <Link underline='hover' color='inherit' href='/diet/diet/'>
            Diet
          </Link>

          <Typography
            sx={{
              color: 'text.primary'
            }}
          >
            {id && urlType === 'copy' ? 'Add new diet' : id && urlType === 'update' ? 'Edit diet' : 'Add new diet'}
          </Typography>
        </Breadcrumbs>
        <Card sx={{ borderBottomLeftRadius: 0, borderBottomRightRadius: 0, boxShadow: 'none' }}>
          <CardContent>
            <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
              <div style={{ width: '90%' }}>
                <Typography variant='h6'>
                  {' '}
                  {id && urlType === 'copy'
                    ? 'Add new diet'
                    : id && urlType === 'update'
                    ? 'Edit diet'
                    : 'Add new diet'}
                </Typography>
              </div>
            </div>
          </CardContent>

          <StepperWrapper sx={{ mb: 5, display: 'flex', justifyContent: 'center' }} className='diet_steps'>
            <Stepper activeStep={activeStep} sx={{ width: '55%', px: 15 }}>
              {steps.map((step, index) => {
                return (
                  <Step key={index}>
                    <StepLabel
                      slots={{
                        // @ts-ignore
                        icon: StepperCustomDot
                      }}
                    >
                      <div className='step-label'>
                        <div>
                          <Typography className='step-title'>{step.title}</Typography>
                          <Typography className='step-subtitle'>{step.subtitle}</Typography>
                        </div>
                      </div>
                    </StepLabel>
                  </Step>
                )
              })}
            </Stepper>
          </StepperWrapper>
        </Card>
        {renderContent()}
      </>
    )
  }

  return (
    <>
      {dietModule && dietModuleAccess === 'ADD' && urlType != 'update' ? (
        <>{contentRenderFunction()}</>
      ) : dietModule && (dietModuleAccess === 'EDIT' || dietModuleAccess === 'DELETE') ? (
        <>{contentRenderFunction()}</>
      ) : (
        <Error404></Error404>
      )}
    </>
  )
}

export default AddDiet
