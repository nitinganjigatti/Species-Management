...
export async function getDietCategoryList(params) {
  const response = await axiosGet({ url: `${DIET_CATEGORIES_LIST}`, params })

  return response.data
}

export async function addDietCategory(payload) {
  try {
...
export async function getDietCategoryById(id) {
  const response = await axiosGet({ url: `${GET_DIET_CATEGORY_BY_ID}/${id}/show` })

  return response.data
}

export async function UpdateDietCategory(id, payload) {
  try {
