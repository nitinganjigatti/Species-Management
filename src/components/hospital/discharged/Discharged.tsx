...
const HospitalDischarged = () => {
  ...
  const [selectedDischargeType, setSelectedDischargeType] = useState<string>('')
      ...
      renderCell: (params: any) => (
        <>
          <Tooltip title={(extractTextFromHtml as any)(params?.row?.discharge_reason || 'NA')}>
            ...
            >
              {(extractTextFromHtml as any)(params?.row?.discharge_reason || 'NA')}
            </Box>
          </Tooltip>
        </>
      )
  ...
  const handleRowClick = async (params: any) => {
    if (params?.field !== 'action') {
      ...
      router.push(`/hospital/discharged/${params.row.id}?${queryParams}`)
    }
  }
