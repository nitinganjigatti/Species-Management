...
interface AddPatientSiteFilterProps {
  localSelections: any
  setLocalSelections: any
}
...
const AddPatientSiteFilter = ({ localSelections, setLocalSelections }: AddPatientSiteFilterProps) => {
  ...
  const [openSectionsListDrawer, setOpenSectionsListDrawer] = useState<boolean>(false)
  const [openEnclosuresListDrawer, setOpenEnclosuresListDrawer] = useState<boolean>(false)

  const zooId = auth?.userData?.user?.zoos?.[0]?.zoo_id

  const handleRemoveSite = (siteId: any) => {
    setLocalSelections((prev: any) => ({
      Sites: prev.Sites.filter((site: any) => site.site_id !== siteId),
      Sections: [],
      Enclosures: []
    }))
  }
  ...
  const handleRemoveSection = (sectionId: any) => {
    setLocalSelections((prev: any) => ({
      ...prev,
      Sections: prev.Sections.filter((section: any) => section.section_id !== sectionId),
      Enclosures: []
    }))
  }
  ...
  const handleRemoveEnclosure = (enclosureId: any) => {
    setLocalSelections((prev: any) => ({
      ...prev,
      Enclosures: prev.Enclosures.filter((enclosure: any) => enclosure.enclosure_id !== enclosureId)
    }))
  }
    ...
    <Box sx={{ display: 'flex', flexDirection: 'column', height: '100%', width: '100%' }}>
      <Card sx={{ border: '1px solid #C3CEC7', boxShadow: 'none', width: '100%' }}>
        <CardHeader
          ...
          {...({ disabled: localSelections?.Sections?.length > 0 || localSelections?.Enclosures?.length > 0 } as any)}
          sx={{
            background:
