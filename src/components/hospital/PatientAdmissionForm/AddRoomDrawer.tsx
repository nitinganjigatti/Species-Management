...
interface AddRoomDrawerProps {
  ...
  updateHospitalStats?: (data: any) => void
}
...
const AddRoomDrawer = ({
  ...
  const fetchAndUpdateHospitalStats = async (hospitalId: any) => {
    ...
    try {
      ...
      if (statsResponse?.success) {
        updateHospitalStats && updateHospitalStats(statsResponse.data)
      }
    } catch (error) {
  ...
  const onAddClick = async (data: FormValues) => {
    setAddLoading(true)
    try {
      const payload = {
      ...
      const res: any = await addHospitalRoom(payload)
      if (res?.success) {
