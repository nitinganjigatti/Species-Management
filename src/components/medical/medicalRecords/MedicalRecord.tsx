...
import { Box, Card, CardHeader, CircularProgress, Grid, IconButton, Typography } from '@mui/material'
...
const MedicalRecords = () => {
    ...
    router.push(
      {
        ...
        query: { ...router.query, animal_id: animal?.animal_id, page: 1 }
      },
      undefined,
      { shallow: true }
    )
  ...
  }, [filters.page, filters.limit, filters.q, selectedAnimal?.animal_id, filterDate])
  ...
  const columns = [
    ...
    },
    {
      ...
      renderCell: (params: any) => (
        <Box sx={{ display: 'flex', alignItems: 'center', gap: 2 }}>
          {params.row.default_icon && <img src={params.row.default_icon} alt='' style={{ width: 20, height: 20 }} />}
          <Typography
            sx={{
              fontSize: '14px',
              color: theme.palette.customColors.OnSurfaceVariant
    ...
    },
    {
      ...
      renderCell: (params: any) => {
        const complaints = params.row.complaint || []
        const text = complaints.map((c: any) => c.complaint).join(', ')
    ...
    },
    {
      ...
      renderCell: (params: any) => {
        const diagnosisList = params.row.diagnosis || []
        const text = diagnosisList.map((d: any) => d.diagnosis || d.name).join(', ')
