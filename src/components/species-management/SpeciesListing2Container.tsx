...
import SpeciesListingView, { type AppliedChip, type PostureStats } from 'src/views/pages/species-management/list2/SpeciesListingView'
...
const SpeciesListing2Container = () => {
  ...
  const handleCellClick = useCallback(
    (params: { field: string; row: Record<string, unknown> }) => {
      if (params.field === 'sl_no' || params.field === 'species_name') {
        router.push(`/species-management/list-2/${params.row.species_id}/`)
      }
    },
    [router]
  )
...
export default SpeciesListing2Container
