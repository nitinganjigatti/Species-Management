...
import SpeciesListingView, { type AppliedChip, type PostureStats } from 'src/views/pages/species-management/ipad2/list/SpeciesListingView'
...
import { buildSpeciesColumns } from 'src/views/pages/species-management/ipad2/list/speciesColumns'
  ...
  const handleCellClick = useCallback(
    (params: { field: string; row: Record<string, unknown> }) => {
      if (params.field === 'sl_no' || params.field === 'species_name') {
        router.push(`/species-management/ipad-2/list/${params.row.species_id}/`)
      }
    },
    [router]
  )
