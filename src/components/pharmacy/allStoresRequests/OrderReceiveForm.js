...
const LabelValues = ({ label, value }) => {
  return (
    <Grid item size={{ md: 2, sm: 3, xs: 6 }} sx={{ pt: 6 }}>
...
function OrderReceiveForm({ shipmentId }) {
  const defaultValues = {
    shipment_id: '',
    // dispatch_id: '',
    // request_id: requestId, need this to be added while doing status update
    request_id: '',
    comments: '',
    store_id: '',
    item_details: [
  ...
  async function updateStatus() {
    ...
    if (receivedItems?.length > 0) {
      ...
      if (verifyCount) {
        ...
        try {
          const result = await updateShipmentRequest(shipmentId, finalReceivedItems)
        ...
        <div>
          <Box sx={{ pb: 6 }}>
            <Grid container justifyContent='space-between'>
              ...
              <Grid container item size={{ xs: 12, sm: 'auto' }} spacing={2}>
                <Grid item>
                  <Button
                    size='large'
                    variant='outlined'
                    ...
                    sx={{ mb: 3.5 }}
                    // component={Link}
                    // href={`/pharmacy/request/${id}/shipment-details?shipmentId=${shipmentId}`}
