...
const Card: React.FC<Props> = props => {
  ...
  const [selectFeed, setSelectFeed] = useState<string>('')
  ...
  const [cutSize, setCutSize] = useState<any>()
  ...
  const handleChangeFeed = (event: any) => {
  ...
  const handleDayClick = (event: any, id: string, title: string) => {
    event.stopPropagation()
    if (id === '0') {
            ...
            <Box>
              ...
              >
                {Day?.map(item => (
                  <Box
                    key={item.id}
                    onClick={event => handleDayClick(event, item.id, item.title)}
                    sx={{
                      fontSize: 11,
                      fontWeight: 'bold',
