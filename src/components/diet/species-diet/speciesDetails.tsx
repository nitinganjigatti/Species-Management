...
interface SpeciesDetailsProps {
  speciesDetailsDrawer: boolean
  setSpeciesDetailsDrawer: (open: boolean) => void
  speciesId: any
  setspeciesId: (id: any) => void
  fetchTableData: () => Promise<void>
}

function SpeciesDetails({ speciesDetailsDrawer, setSpeciesDetailsDrawer, speciesId, setspeciesId, fetchTableData }: SpeciesDetailsProps) {
  ...
  const [specieDetails, setSpecieDetails] = useState<any>({})

  const [status, setStatus] = useState<string>('1')

  const [uploadDietDrawer, setUploadDietDrawer] = useState<boolean>(false) // has to be modified
  ...
  const removeAttachment = async (speciesId: any, attachment_id: any) => {
    if (attachment_id) {
      setDetailsLoader(true)
      try {
  ...
  const speciesAttachmentActiveFunc = async (speciesId: any, attachmentId: any) => {
  ...
  const DietitianAvatar = ({ item }: { item: any }) => {
          ...
          <>
            ...
            <TabContext sx={{ width: '100%' }} value={status}>
              ...
              <TabPanel value='1'>
                ...
                >
                  {specieDetails?.active_attachments?.map((item: any, index: number) => (
              ...
              <TabPanel value='0'>
                ...
                >
                  {specieDetails?.deactive_attachments?.map((item: any, index: number) => (
