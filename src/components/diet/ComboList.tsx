...
interface Props {
  ...
  setSelectedCardCombo: (...args: any[]) => any
  selectedCardCombo: any
  checkid: any
  onChange: (...args: any[]) => any
  allComboSelectedValues: any
  setallComboSelectedValues: (...args: any[]) => any
  formData: any
  fromrow: any
  comboid: any
  cutsizelist: any
...
const ComboList: React.FC<Props> = props => {
  const {
    addEventSidebarOpen,
    handleSidebarClose,
    ...
    const getRecipeListData = async () => {
      ...
      if (res?.data?.result?.length > 0) {
        const newResults = res.data.result.filter(
          (item: any) => !ingredientList.some(existingItem => existingItem.id === item.id)
        )
