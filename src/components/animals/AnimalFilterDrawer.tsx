...
const AnimalFilterDrawer: React.FC<AnimalFilterDrawerProps> = ({
  ...
  const handleAnimalTypeChange = useCallback((value: string) => {
    setSelectedOptions(prevOptions => {
      const newOptions = {
        ...prevOptions,
  ...
  const handleDateRangeChange = useCallback((startDate: Date | string, endDate: Date | string) => {
    setSelectedOptions(prevOptions => {
      const newOptions = {
        ...prevOptions,
      ...
      {selectedMenu === 'Accession Date' && (
        <Box sx={{ display: 'flex', flexDirection: 'column', height: '100%' }}>
          <Typography sx={{ fontSize: '14px', color: theme.palette.customColors.Outline, mb: 3 }}>
            Select a date range for accession date
          </Typography>
