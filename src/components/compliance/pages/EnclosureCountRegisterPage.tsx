...
const EnclosureCountRegister = () => {
  ...
  const siteIdsArr = (
    selectedItems?.Site?.length
      ? selectedItems.Site
      : selectedSite?.site_id || siteIdParam
      ? [String(selectedSite?.site_id || siteIdParam)]
      : []
  ).map(String)
  ...
  const downloadEnclosureCountRegister = async () => {
    ...
    if (!siteIdsDownload.length && (selectedSite?.site_id || siteIdParam)) {
      siteIdsDownload.push(String(selectedSite?.site_id || siteIdParam))
    }
...
export default EnclosureCountRegister
