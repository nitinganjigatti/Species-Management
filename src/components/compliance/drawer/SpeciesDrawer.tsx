...
const SpeciesDrawer = ({ open, onClose, data, onSelect, selectedSpecies = [], title = 'Select Species' }: SpeciesDrawerProps) => {
  ...
  const [newlySelectedAntzItems, setNewlySelectedItems] = useState<Species[]>([])
  ...
  const handleToggle = (speciesItem: Species) => {
    setNewlySelectedItems([speciesItem])
  }

  const handleAddCustomSpecies = (newSpecies: Species) => {
    setNewlySelectedItems([newSpecies])
    handleDone(newSpecies)
  }

  const handleDone = (specieItem?: Species) => {
          ...
          {data?.name && (
            ...
            >
              <CellInfo
                ...
                avatarUrl={undefined}
                inchagename=''
                defaultImage='/images/default_specie_icon.png'
                defaultImageAlt={data?.name || ''}
              />
            </Box>
          )}
