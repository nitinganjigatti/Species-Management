...
const getGenderChipProps = (gender: string | undefined, theme: Theme) => {
  switch (gender) {
    case 'male':
      return {
        label: 'M',
            ...
            <Box sx={{ px: 4, py: 5 }}>
              {loading ? (
                <Grid container spacing={2} rowSpacing={4}>
                  {[...Array(6)].map((_, idx) => (
                    <StyledGrid size={{ xs: 12, sm: 6 }} key={idx}>
...
const StyledTypography = styled(Typography)<StyledTypographyProps>(({ theme, fontSize, fontWeight, color }) => ({
