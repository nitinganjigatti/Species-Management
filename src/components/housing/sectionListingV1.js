...
const SectionListing = () => {
  ...
  const { loading, total, page, pageSize, sortBy, sortOrder, search } = useSelector(state => state.section)

  const cacheKey = `${id}${search}_${sortBy}_${sortOrder}_${page}`
  const sectionList = useSelector(state => state.section.pages[cacheKey] || [])
