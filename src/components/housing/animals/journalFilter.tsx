            ...
            {categories.map(menu => (
              <Box
                ...
                sx={{
                  ...
                  '&:hover': {
                    backgroundColor:
                      activeCategory === menu ? theme.palette.customColors?.OnPrimary : theme.palette.grey[100]
                  }
                }}
                onClick={() => {
                  ...
                  <Box sx={{ display: 'flex', alignItems: 'center', mt: 2 }}>
                    <Checkbox
                      checked={
                        ...
                        Array.isArray(selectedOptions[activeCategory as keyof SelectedOptions]) &&
                        (selectedOptions[activeCategory as keyof SelectedOptions] as any[])?.length ===
                ...
                <Box sx={{ ml: 2 }}>
                  {isLoader ? (
                    ...
                    filteredOptions.map((option, index) => (
                      <Box
                        key={index}
                        sx={{ cursor: 'pointer', display: 'flex', alignItems: 'center', gap: '8px', mb: 2 }}
                      >
                        <Checkbox
                    ...
                    filteredOptions.map((option, index) => (
                      <Box
                        onClick={() => handleToggleOption((option as CategoryOption).categoryId, activeCategory)}
                        key={index}
                        sx={{ cursor: 'pointer', display: 'flex', alignItems: 'center', mb: 2 }}
                      >
                        <Checkbox
                          checked={(selectedOptions[activeCategory] || []).includes(
                            (option as CategoryOption).categoryId
                          )}
