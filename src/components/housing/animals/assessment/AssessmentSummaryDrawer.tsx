...
import { getAssessmentHistory } from 'src/lib/api/assessment'
...
interface AssessmentSummaryDrawerProps {
  open: boolean
  onClose: () => void
...
const AssessmentSummaryDrawer: React.FC<AssessmentSummaryDrawerProps> = ({
  open,
  onClose,
  assessmentTypes,
  ...
  const fetchHistory = useCallback(
    async (page: number, reset: boolean = false) => {
      if (!selectedTypeId || !animalId) return

      setIsLoading(true)
      try {
        const response = await getAssessmentHistory(animalId, {
    ...
    [selectedTypeId, animalId, historyEntries.length]
  )
...
export default AssessmentSummaryDrawer
