...
interface MedicalJournalDrawerProps {
  open: boolean
  onClose: () => void
...
interface JournalCardProps {
  ...
  formatDetailValue: (key: string, value: unknown) => string | null
  isLast: boolean
}
...
const formatDetailValue = (key: string, value: unknown): string | null => {
...
const JournalCard: FC<JournalCardProps> = ({ entry, theme, formatKey, formatDetailValue, isLast }) => {
...
const MedicalJournalDrawer: FC<MedicalJournalDrawerProps> = ({ open, onClose, animalId, medicalRecordId }) => {
  ...
  const fetchJournalData = async (pageNo: number): Promise<void> => {
    try {
      if (pageNo === 1) {
        setLoading(true)
      } else {
      ...
      if (res?.success) {
        const newData: JournalGroup[] = res?.data?.data || []
              ...
              <Box key={groupIndex} sx={{ mb: 3 }}>
                ...
                {group?.entries?.map((entry, entryIdx) => (
                  <JournalCard
                    ...
                    isLast={entryIdx === (group.entries?.length || 0) - 1}
                  />
                ))}
              </Box>
            ))}
...
export default memo(MedicalJournalDrawer)
