...
interface SelectOrganDrawerProps {
  open: boolean
  setOpen: (open: boolean) => void
  selectedOrgans: SelectedOrgan[]
  onAddSelected?: (organs: GroupedOrgan[]) => void
}
...
const SelectOrganDrawer: FC<SelectOrganDrawerProps> = ({ open, setOpen, selectedOrgans, onAddSelected }) => {
  ...
  const [organCategories, setOrganCategories] = useState<OrganCategory[]>([])
  ...
  const initializeSelection = (): void => {
    const selected: LocalSelectedItem[] = []
    selectedOrgans.forEach(organ => {
      if (organ.parts && organ.parts.length > 0) {
  ...
  const fetchOrgans = async (): Promise<void> => {
    setLoading(true)
    try {
  ...
  const categoryTabs = useMemo<CategoryTab[]>(() => {
    ...
    const tabs: CategoryTab[] = [{ id: '', label: 'All', count: totalParts }]
  ...
  const activeParts = useMemo<OrganPart[]>(() => {
    if (activeCategory === '') {
      const allParts: OrganPart[] = []
      organCategories.forEach(cat => {
  ...
  const filteredParts = useMemo<OrganPart[]>(() => {
  ...
  const isPartSelected = (part: OrganPart): boolean => {
  ...
  const handleTogglePart = (part: OrganPart): void => {
    const partId = String(part.id || part.body_part_id)
  ...
  const handleAdd = (): void => {
    const groupedByCategory: Record<string, GroupedOrgan> = {}
