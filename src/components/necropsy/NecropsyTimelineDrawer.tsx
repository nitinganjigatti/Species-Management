...
import { useTheme, styled, alpha, Theme } from '@mui/material/styles'
import {
  Close as CloseIcon,
...
interface NecropsyTimelineDrawerProps {
  open: boolean
  onClose: () => void
...
const NecropsyTimelineDrawer: FC<NecropsyTimelineDrawerProps> = ({ open, onClose, mortalityId }) => {
  ...
  const [rawData, setRawData] = useState<TimelineEntry[]>([])
  ...
  const fetchTimeline = useCallback(
    async (pageNo: number = 1): Promise<void> => {
      ...
      try {
        ...
        if (res?.success && Array.isArray(res.data?.result)) {
          ...
          const grouped = merged.reduce<Record<string, TimelineSection>>((acc, item) => {
...
export default memo(NecropsyTimelineDrawer)
