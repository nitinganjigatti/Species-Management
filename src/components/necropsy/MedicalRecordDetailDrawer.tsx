...
const SectionHeader: FC<SectionHeaderProps> = ({ icon: IconComponent, title, theme }) => (
  <Box sx={{ display: 'flex', alignItems: 'center', gap: 1.5, mb: 2 }}>
    <IconComponent
      sx={{ fontSize: 20, color: theme.palette.customColors?.neutralSecondary || theme.palette.text.secondary }}
    />
    <Typography
      sx={{
        fontSize: '14px',
...
const MedicalRecordDetailDrawer: FC<MedicalRecordDetailDrawerProps> = ({ open, onClose, medicalRecordId }) => {
  ...
  const [labDetailsDrawerOpen, setLabDetailsDrawerOpen] = useState<boolean>(false)
  const [selectedLabRequest, setSelectedLabRequest] = useState<LabItem | null>(null)
  ...
  const fetchDetails = async (): Promise<void> => {
    try {
      setLoading(true)
      const res = await getMedicalRecordDetails(Number(medicalRecordId))
      if (res?.success && res?.data) {
        setData(res.data as unknown as MedicalRecordData)
      }
    } catch (error) {
  ...
  const getAvailableTabs = (): TabIcon[] => {
    if (!data) return []
    ...
    return TAB_ICONS.filter(tab => {
      switch (tab.id) {
        ...
        case 'complaints':
          return (data?.complaints?.length || 0) > 0
        case 'diagnosis':
          return (data?.diagnosis?.length || 0) > 0
        case 'prescription':
          return (data?.prescription?.length || 0) > 0
        case 'advice':
          return (data?.advices?.length || 0) > 0
        case 'lab':
          return (data?.lab?.length || 0) > 0 || (data?.lab_data?.length || 0) > 0
        case 'attachments':
          return (
            (data?.notes?.images?.length || 0) > 0 ||
            (data?.notes?.documents?.length || 0) > 0 ||
            (data?.notes?.videos?.length || 0) > 0
          )
        case 'notes':
          return (data?.notes?.notes?.length || 0) > 0
        case 'followup':
  ...
  const renderTabContent = (): React.ReactElement | null => {
    switch (activeTab) {
      ...
      case 'attachments': {
        const allFiles = [
          ...(data?.notes?.images?.map(f => ({ ...f, fileType: 'image' })) || []),
          ...(data?.notes?.documents?.map(f => ({ ...f, fileType: 'document' })) || []),
          ...(data?.notes?.videos?.map(f => ({ ...f, fileType: 'video' })) || [])
        ]
              ...
              >
                {allFiles.map((file, idx) => (
                  <NewMediaCard
                    ...
                    fileName={file.file_original_name || 'File'}
  ...
  return (
    ...
    >
                  ...
                  {data.user_details?.user_mobile_number && (
                    <>
                      ...
                      <Box sx={{ display: { xs: 'flex', md: 'none' }, gap: 1 }}>
                        <IconButton
                          size='small'
                          onClick={() => window.open(`tel:${data.user_details?.user_mobile_number}`, '_self')}
                        ...
                        <IconButton
                          size='small'
                          onClick={() => window.open(`sms:${data.user_details?.user_mobile_number}`, '_self')}
                          sx={{ bgcolor: theme.palette.customColors?.surfaceVariant || theme.palette.grey[100] }}
                        >
                        ...
                        >
                          ...
                          <Tooltip title={copied ? t('necropsy_module.copied') : t('necropsy_module.copy_number')}>
                            <IconButton
                              ...
                              sx={{
                                '&:hover': {
                                  bgcolor: theme.palette.customColors?.surfaceVariant || theme.palette.grey[200]
                                }
                              }}
                            >
                              <Icon icon={copied ? 'mdi:check' : 'mdi:content-copy'} fontSize={16} />
                            </IconButton>
                          </Tooltip>
                        </Box>
                      </Box>
      ...
      <MedicalJournalDrawer
        ...
        animalId={animalData?.animal_id ? Number(animalData.animal_id) : null}
