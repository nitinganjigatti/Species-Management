                  ...
                  render={({ field }) => (
                    <>
                      <Autocomplete
                        ...
                        renderInput={params => <TextField {...params} label='Users*' error={Boolean(errors.user_id)} />}
            ...
            <TableBody>
              {productArrayUi.length > 0
                ? productArrayUi.map((el, index, array) => {
                    ...
                    const isFirstRow = index === array.findIndex(item => item?.stock_id?.label === el?.stock_id?.label)
                    return (
                      <TableRow key={index}>
                        {isFirstRow && (
                          <TableCell
