  ...
  const onValid = async data => {
    ...
    try {
      ...
      if (response?.status === true) {
        ...
        fetchClinPathList()
        router.push(
          `/hospital/outpatient/${id}/AddAnesthesiaRecord/?tab=anesthesia&anaesthesia_id=${response?.data?.anaesthesia_id}`
        )
      } else {
    ...
    <FormProvider {...methods}>
      <Box display='flex' flexDirection='column' gap={3} sx={{ p: 3 }}>
        <Breadcrumbs aria-label='breadcrumb'>
          ...
          <Typography color={theme.palette.text.secondary}>Outpatient</Typography>
