...
const ActivityLogs = ({ handleSidebarClose, searchValue, setSearchValue, FeedDetailsValue }) => {
  ...
  const getActivityLogs = async searchVal => {
    try {
      const params = {
        activity_type_id: FeedDetailsValue?.id,
  ...
  return (
    <Box sx={{ display: 'flex', flexDirection: 'column', gap: '8px' }}>
      <Box sx={{ py: 4, position: 'sticky', top: 0, backgroundColor: '#fff', zIndex: 100 }}>
        <Box
