...
rm -f .env;
# .env is generated by the GitHub build from GitHub Environment vars and
# shipped inside the build artifact (.next/deploy.env). It is restored below,
# after the artifact is downloaded and unzipped.
...
# Restore .env from the build artifact (single source of truth: GitHub Environment vars)
cp .next/deploy.env .env
rm -f .next/deploy.env
ls -la
