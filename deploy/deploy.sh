...
elif [ $GITHUB_REF_NAME == 'staging-release' ]
then
...
do
  echo "deploying to ${server}"
  ssh -o UserKnownHostsFile=/github/home/.ssh/known_hosts antzsystems@${server} 'bash -s' < ./deploy/fetchRepo.sh $GITHUB_REF_NAME $ENV_TO_LOAD
done
