...
**Currently on `@antzsoft/chat-core@1.2.5`** (v1.2.5 decouples socket and REST under the same transit-encryption security pattern; adds `@PreTransit()` decorator on `/users/me`, `/users/me/devices`, and `/storage/files/upload` so they bypass the transit handshake during bootstrap; transit-aware proxy now returned from `getSocket()` / `tryGetSocket()` — see [package.json](../../../package.json)).
...
8. **`GET /conversations` omits `senderId` + `sender.displayName` + `deliveryStatus` on `lastMessage`.** Returns `senderId: ''` (empty string), no `sender` object, and no `deliveryStatus` on the embedded last message. Affects two sidebar features:
   - WhatsApp-style "Saket: …" sender prefix for groups can't render on cold load
   - Sidebar read-receipt tick (sent / delivered / seen) shows the wrong state — defaults to single-check sent regardless of actual delivery state — for BOTH DMs and groups

   We work around with the `enrichLastMessageSenders` thunk fired after every `fetchChatsContacts` (in both `AppChat.tsx` and `ChatLauncher.tsx`). For each chat where the lastMessage is missing sender info OR has incomplete feedback on a likely-own message, it fires `getMessage(id)` REST in parallel and backfills via the extended `patchLastMessageSender` reducer (now accepts an optional `feedback` field alongside the sender info). Session-scoped `enrichedLastMessageIds` Set prevents refetching the same message twice. After the initial backfill, **all live updates flow via sockets** (`new_message`, `read_receipt`, `message_delivered`) — the reducer guard `chat.lastMessage.feedback` now always passes because the adapter initializes the field with defaults, so socket updates correctly mutate `lastMessage.feedback.isDelivered` / `isSeen` in place.

   **Cleanup**: when backend ships sender + deliveryStatus on the conv-list response, the `enrichLastMessageSenders` thunk + the `feedback` param on `patchLastMessageSender` self-deprecate (targets filter to empty, callers stop passing feedback). Grep `REMOVE ONCE BACKEND` to find the marker.
...
11. **`user_offline` event payload omits `lastSeenAt`.** Verified via diagnostic logs 2026-05-22 — every `user_offline` broadcast arrives with `lastSeenAt: undefined`. The SDK's listener has `if (e.lastSeenAt) store.setLastSeen(...)` so the store update is skipped, and live offline transitions leave the UI stuck on whatever was cached. We work around by listening to the event ourselves in [`AppChat.tsx`](../../../src/views/apps/chat/AppChat.tsx) and refetching via the existing `getUserLastSeen(userId)` REST endpoint — no client-side timestamp fabrication, server stays authoritative. Also dedupes the per-room 5×-fanout (server emits one event per joined room within ~80ms, same userId — we swallow duplicates inside a 1s window per userId). If the server later includes `lastSeenAt` in the payload, our handler trusts it directly via `evt.lastSeenAt ?? <REST refetch>` — no further client change needed.
12. **`users.lastSeenAt` stamped on activity heartbeats, not on socket disconnect.** REST `getUserLastSeen(userId)` returns the timestamp of the user's last *activity* (last message sent / typing event / periodic heartbeat), not the moment their socket disconnected. Verified by comparing actual disconnect time vs REST return value — typically 1–5 min stale depending on how idle the user was before disconnecting. Backend fix needed: stamp `users.lastSeenAt = now()` on socket disconnect handler. Until then, "last seen X" is best understood as "last seen active X" — close enough for product UX but not millisecond-accurate.

13. **`POST /conversations/direct` always inserts instead of find-or-create — produces duplicate DM rows.** Verified 2026-05-29 via instrumented `[chat:dup-trace]` logs (since removed). Server returned 2 distinct `direct` conversation rows for the pair `(6a1081e24cc81ec6f9e329cb, 6a1081e34cc81ec6f9e32a3f)`: id `6a11728a2eb1915f8f267eef` (created 2026-05-23T09:25Z by Anil) and id `6a1177696dd081fadc18d8e5` (created 2026-05-23T09:46Z by Anil, 21 min later). Both have real message activity — neither is empty. **Server fix needed**: on the endpoint, query first for an existing `direct` conversation where `participants.userId` contains both the caller and the requested `userId` and `participantCount === 2`; return that row if found; insert only if none. **Migration also needed**: for each `(userA, userB)` pair with multiple `direct` rows, keep the row with most recent `updatedAt`, repoint every `messages.conversationId` from the obsolete rows to the kept one, then delete the obsolete rows. Frontend cannot dedupe client-side without silently hiding real messages — so this MUST be fixed server-side. **Frontend behavior today**: sidebar dedupes by `conversation.id`, so duplicate DM rows render as separate entries (both show the peer's display name → looks like "Nidhin Pratap × 2"). Adapter at [src/lib/chat/api.ts:983](../../../src/lib/chat/api.ts#L983) correctly distinguishes group vs direct via `conversationType === 'group'`.

14. **Multi-participant conversations sometimes mis-flagged as `direct` / missing `conversationType`.** Same 5/29 diagnostic dump showed conversations with 3, 4, 5, 7, even 10 participants where `conversationType !== 'group'`. A DM is by definition 2 participants — multi-participant rows must carry `conversationType: 'group'`. **Server fix**: set `conversationType: 'group'` whenever `participants.length > 2`; backfill existing rows. Until fixed, our `startDirectChat` existing-DM lookup (`if (c.isGroup === true) return false`) lets these leak through as DM candidates — safe for the count-2 case but the adapter renders them with peer-name fallback rather than the group name.
...
- **Backend: `read_receipt` / `message_delivered` / `conversation_updated` not reliably emitted to sender on DM read** — verified via socket-event audit: refresh always pulls the correct `isSeen: true` from `GET /conversations`, but no socket event arrives live. Workaround (2026-06-02): receipt-sync re-fetches the conversation list on tab-focus return after >5s hidden. REMOVE the workaround once backend emits the events reliably. Tracked in [ChatClientContext.tsx](../../../src/contexts/ChatClientContext.tsx) — search for `REMOVE ONCE BACKEND`.
- **Backend: `@PreTransit()` coverage incomplete on chat REST endpoints (SDK 1.2.5).** Endpoints called during the transit-ECDH-setup window (~few seconds after socket reconnect) return `403 Transit session required`. Affected: `GET /conversations`, `GET /conversations/{id}/messages`, `GET /conversations/{id}/read-receipt`, `GET /messages/{id}`, `POST /users/me/update`, `POST /users/me/avatar/sync`. The 1.2.5 release marks `/users/me`, `/users/me/devices`, `/storage/files/upload` with `@PreTransit()` but the rest are missing. Workaround (2026-06-02): receipt-sync delays its initial fetch by 2s to dodge the race. Proper fix: extend `@PreTransit()` to all chat REST endpoints reachable before the transit handshake completes, OR have the SDK queue outbound REST until the transit session is established.
- **Backend: 403 (not 401) for stale chat tokens.** Global axios interceptor in [src/lib/api/utility/index.js](../../../src/lib/api/utility/index.js) listens for 401 → dispatches `'session-expired'` event → AuthContext logs the user out. Chat REST returns 403 for stale tokens, which bypasses that flow — user stays on the page with a broken chat until manual refresh. Backend should align with HTTP semantics (401 for invalid/expired, 403 for valid-but-forbidden) so the global handler picks it up.
...
- **2026-06-02** — Chat resilience overhaul — offline-safe sends, SDK self-heal, receipt-sync workarounds, and a handful of UI polish fixes. Built defensively in phases so each step is independently shippable; all wired to existing socket lifecycle events with no new event bus or persistence layer.
  - **Pending outbox + auto-retry.** New `state.pendingOutbox: PendingOutboxEntry[]` field. When `sendMsg` rejects (typically `[AntzChat] Socket reconnect failed`), the `.rejected` builder queues the failed attempt with conversationId + text + attachments + replyTo + tempId. New `flushPendingOutbox` thunk replays entries in FIFO order on socket recovery — fires from BOTH `recovery ✓` (transit-encryption fix path) AND `reconnect ✓ (auto)` (clean Socket.IO auto-reconnect) so any successful (re)connect drains the queue. Idempotent (no-op on empty outbox), `__fromOutbox: true` marker prevents retry-on-retry re-queueing.
  - **SendMsgForm hardening.** Composer text + attachments clear **synchronously** before the `await dispatch(sendMsg(...)).unwrap()` so double-tap Enter / fast double-click can't dispatch twice with the same text. On rejection, captured text + previews are restored AND a `queuedRef` records what was queued — a `useSelector` on `pendingOutbox` then auto-clears the composer when the queued entry is successfully retried by recovery (prevents the "I see text in composer, must be unsent" duplicate-send loop).
  - **Phase #2 async-wrap of REST exports.** Converted 56 `function X(): Promise<...>` exports in [src/lib/chat/api.ts](../../../src/lib/chat/api.ts) to `async function X()` so the sync throw from `requireClient(...)` becomes a Promise rejection instead of an "Uncaught Error" runtime overlay. Eliminates the `[chat-api] X: SDK not initialized` crash that previously surfaced when a useEffect (e.g. `ChatContent.useEffect → getUserLastSeen`) fired during the SDK init gap after a long sleep. The `requireClient` docstring now explicitly states the calling contract: any `Promise<X>`-returning caller MUST be declared `async`.
  - **Phase #3 SDK self-heal on long sleep.** New `lastHiddenAtRef` + `reinitCounter` state in [ChatClientContext](../../../src/contexts/ChatClientContext.tsx). On visibility return after >5 min hidden AND `getChatClientOrNull() === null`, bumps `reinitCounter` (which is now in the main init effect's dep list) to force a full SDK + socket rebuild. Idempotent via `reinitInFlightRef`. Silent retry with backoff (5s → 10s → 20s) on rebuild failure; gives up after 3 attempts. On success, dispatches `fetchChatsContacts` to replay the conversation list so the sidebar reflects whatever arrived during the dead-SDK window.
  - **Receipt-sync workaround for missing socket events.** Verified via socket-event audit that the backend does NOT reliably emit `read_receipt` / `message_delivered` / `conversation_updated` to the sender when the recipient reads a DM (refresh always shows ✓✓, live never updates). Workaround: when the tab returns from >5s hidden, silently `dispatch(fetchChatsContacts())` to pull missed feedback. Initial fetch is delayed 2s (`RECEIPT_SYNC_INITIAL_DELAY_MS`) to let the transit ECDH session re-establish post-reconnect — works around incomplete `@PreTransit()` coverage on the backend (SDK 1.2.5 only marks `/users/me`, `/users/me/devices`, `/storage/files/upload`; chat REST endpoints still 403 when called during the transit-setup race). Retry once after 3s on failure. 30s cooldown prevents spam during rapid focus changes. `fetchChatsContacts` thunk extended with an optional `{ rethrow?: boolean }` arg so the receipt-sync path can detect failures via `.unwrap().catch()` — existing callers (ChatLauncher, AppChat, UserProfileRight, self-heal replay) pass no arg and keep the legacy swallow-and-return-empty contract.
  - **`patchConversationFromEvent` feedback lookup.** When `conversation_updated` rebuilds `lastMessage`, prefers the live `messages[i].feedback` (which the receipt reducer has been mutating in-place) over the inherited/default. Closes the race where a receipt event lands before `conversation_updated` and the rebuild resets the sidebar tick to ✓ even though the bubble shows ✓✓. Falls back to existing/default if the message isn't in the messages array.
  - **Transit-error regex extended.** `isTransitHandshakeFailure` now catches the `Transit session not found or expired — reconnect to create a new session` variant in addition to the original `Transit encryption required: missing transitEphemeralPub/transitAlgo in handshake auth`. Same recovery action for both. Add more variants to the alternation as the backend introduces them.
  - **Pinned-message highlight restored.** The `.msg-flash` CSS keyframe + class was missing from [styles/custom.css](../../../styles/custom.css) — `vantara-chat` branch diverged before the polish commits (`0cda54106`, `cc559cfb7`) that added it on `dev-release`. Restored the polished version (2200ms cubic-bezier, alpha background tint + matching box-shadow ring in brand primary green). Also extended JS `setTimeout` in [ChatLog.tsx](../../../src/views/apps/chat/ChatLog.tsx) from 1200ms → 2200ms so the class removal matches the animation duration (no mid-fade cut-off).
  - **Pinned-strip forward-marker stripping.** [PinnedMessagesStrip](../../../src/views/apps/chat/PinnedMessagesStrip.tsx) now imports `isForwarded` / `stripForwardMarker` from [src/lib/chat/forwardMarker.ts](../../../src/lib/chat/forwardMarker.ts). Forwarded pinned messages render a `mdi:share` icon + the body with the sentinel stripped, matching the in-bubble `ForwardedTag` treatment. Was showing literal `[fwd]` text before.
  - **FilterChip count badge hover legibility.** [FilterChip](../../../src/views/utility/FilterChip.tsx) — when the inactive chip is hovered the bg turns dark teal; the count badge bg (also dark teal) was previously camouflaged. Added a `.filter-chip-count-badge` className + parent-hover descendant rule that swaps the badge to translucent-white on hover. Active-state styling unchanged.
  - **Verbose lifecycle logs stripped.** Removed `[chat:socket-trace] connect ✓ / reconnect ✓ (auto) / disconnect ✗ / reconnect_attempt / reconnect_error / tab active / recovery skipped` plus `[chat:selfheal] long-sleep SDK alive / re-init already in flight / replay ✓` plus `[chat:receipt-sync] focus return → refreshing / retry → refreshing`. Kept all `console.warn` / `console.error` paths (real failures), the `[chat:gate]` init logs, the `[chat:socket] recovery →` warn (rare + meaningful), the `[chat:selfheal] re-init ✓` (fires once per recovery cycle), and the outbox logs (`queued failed send`, `flush start`, `flush ok`, `retry failed — skip re-queue`) which only fire on real outbox actions. Net: production console stays quiet during normal use; errors + significant one-offs still surface with enough context to debug.
  - **Backend tickets opened** (see "Risks / Known gaps"):
    - `read_receipt` / `message_delivered` / `conversation_updated` socket events not reliably emitted to sender on DM read — workaround above.
    - `@PreTransit()` coverage incomplete on chat REST endpoints (`/conversations`, `/conversations/{id}/messages`, `/conversations/{id}/read-receipt`, `/messages/{id}`, `/users/me/update`, `/users/me/avatar/sync`) — endpoints called during the transit-setup window return `403 Transit session required` after socket reconnect. The 2s receipt-sync delay dodges most of this race, but a clean backend fix requires extending the decorator OR client-side queuing of REST until transit is ready.
    - 403 (not 401) returned for stale chat tokens — the existing global axios interceptor only catches 401 → session-expired, so a 403 from chat REST doesn't trigger logout. Backend should align with HTTP semantics (401 for expired/invalid, 403 for valid-token-no-permission) so the global interceptor picks it up.

- **2026-05-22c** — Chat UI polish — secondary-palette migration across surfaces:
  - **Group info action buttons hidden.** Add / Search action buttons in `UserProfileRight` group view commented out (duplicates: "Add" → Members section header, "Search" → chat header search icon → `SearchMessagesDrawer`). Code preserved verbatim inside a JSX comment for easy re-enable.
  - **Add Member affordances → secondary.** "Add member" row in [`UserProfileRight`](../../../src/views/apps/chat/UserProfileRight.tsx) Members section (circle bg + text), and "Add Member" button in [`ChatLog`](../../../src/views/apps/chat/ChatLog.tsx) group-created card (bg `customColors.antzSecondaryBg`, text + icon `secondary.main`).
  - **Member list cosmetics → secondary.** Row hover `customColors.Surface` → `customColors.antzSecondaryBg` (light teal). "Group admin" badge bg `primary.dark` → `secondary.dark` (dark teal/navy).
  - **Group + DM info row hovers → teal.** Media, Starred row hovers flipped from `customColors.Surface` to `customColors.antzSecondaryBg` (replace_all over 9 instances in [`UserProfileRight`](../../../src/views/apps/chat/UserProfileRight.tsx)).
  - **All Switches `color='secondary'`** — Mute / Pin (group + DM) + Add-as-admin (inline panel + standalone drawer). When toggled on the thumb + track render teal instead of MUI default primary green.
  - **Media drawer Tabs → secondary.** [`MediaLinksDocsDrawer`](../../../src/views/apps/chat/MediaLinksDocsDrawer.tsx) Tabs gain `textColor='secondary'` + `indicatorColor='secondary'`; "Load more" links flip `primary.main` → `secondary.main`.
  - **Group-info edit affordances → secondary.** Edit pencil IconButton next to group name explicit `color: 'secondary.main'`; Save button (check icon) border + color → `secondary.main`; "Add group description" link `primary.main` → `secondary.main`. TextField focused-state `:after` underline added with `secondary.main` and hover `customColors.Outline` (both name + description fields).
  - **Inline Add-members panel → secondary.** Selected-avatar overlay (`primary.main` → `secondary.main`), trailing check-circle (`primary.main` → `secondary.main`), "Add (N)" footer Button gains `color='secondary'`, "{N} added" pill bg `customColors.Surface` → `customColors.antzSecondaryBg` and text `primary.dark` → `secondary.dark`.
  - **Standalone [`AddMembersDrawer`](../../../src/views/apps/chat/AddMembersDrawer.tsx) brought to parity.** Same 6 changes mirroring the inline panel — single visual identity across both add-members surfaces.
  - **Bug fix — Add-members panel state leak on outside-click close.** Two new effects in [`UserProfileRight`](../../../src/views/apps/chat/UserProfileRight.tsx): one resets `addingMembers` / `selectedToAdd` / `addQuery` / `addAsAdmin` when `userProfileRightOpen` flips to false (clicking outside the drawer used to leave stale selections); another resets on `contactId` change so switching to another group mid-flow clears the panel state.
  - **Bug fix — 3-dots IconButton hit-area + click propagation.** Member-row kebab IconButton now calls `e.stopPropagation()` on click + mousedown so the row's onClick (open DM) doesn't fire alongside the menu, and `sx={{ p: 1.5 }}` widens the touch target to ~44×44 (WCAG 2.5.5 / Material's minimum touch-size guideline).

- **2026-05-22b** — WhatsApp-style deferred direct-conversation creation + unread divider top-level centering + `formatLastSeen` extracted to shared util:
  - **Deferred direct-conversation (WhatsApp parity).** Clicking a new user from the compose popover no longer eagerly fires `POST /conversations/direct`. The chat panel opens against a local-only "draft" `selectedChat`; the server-side `Conversation` is materialized only when the user actually sends their first message. If they navigate away without sending, the draft just disappears — no orphan conversation, no sidebar row, no server state.
    - **Type change**: new optional `isDraft?: boolean` flag on [`ChatsArrType`](../../../src/types/apps/chatTypes.ts) — strictly additive.
    - **Two new reducers** in [`store/apps/chat/index.ts`](../../../src/store/apps/chat/index.ts):
      - `setDraftChat(chat)` — sets `state.selectedChat = { chat, contact }` and DOES NOT add to `state.chats` (no sidebar row for unsent drafts).
      - `materializeDraft(realChat)` — inserts the real server-backed row at top of `state.chats`; if the open chat is a draft, repoints `selectedChat` to the real conversation so the open panel keeps working with the real id (preserves any composer state — text input, attachments queue, reply pill — across the swap).
    - **New idempotent thunk `materializeDraftIfNeeded(contact)`** — single point of materialization. If `contact.isDraft`, calls `createDirectConversation({ userId: peer })`, dispatches `materializeDraft`, returns the real id. Else returns the existing id verbatim. Used by both `sendMsg` (for text/mixed sends) and [`SendMsgForm`](../../../src/views/apps/chat/SendMsgForm.tsx) (before the attachment-upload presigned-URL request — that hop needs a real conversation id, the `__draft__<userId>` placeholder would 404).
    - **`startDirectChat` thunk rewritten** in [`store/apps/chat/index.ts`](../../../src/store/apps/chat/index.ts): existing-DM lookup (`state.chats` filtered by `!isGroup && participantIds.includes(currentUserId, peerUserId)`) → `selectChat(existing.id)` and return (REST-free reuse). No existing DM → `getUserById(peerId)` to populate name/avatar, build draft `ChatsArrType` with `id: __draft__<peerUserId>`, `isDraft: true`, `participantIds: [me, peer]`, empty `chat.messages` → dispatch `setDraftChat(draft)`. No `createDirect` call.
    - **`sendMsg` thunk** detects `obj.contact?.isDraft` and calls `materializeDraftIfNeeded(obj.contact).unwrap()` to swap to the real id BEFORE emitting `send_message`. From there the rest of the thunk runs unchanged — the ack lands the message in the now-real conversation (matched by id in the reducer).
    - **Network footprint**: opening a new user costs exactly one `GET /users/:id` (display info — no different from WhatsApp loading the contact's profile photo when you tap their name). `POST /conversations/direct` is only ever called on the first send.
    - **Affected components, byte-for-byte review**: [`SidebarLeft`](../../../src/views/apps/chat/SidebarLeft.tsx) (contact click still dispatches `startDirectChat` — no change). All other flows (group create, message send/receive, conversation update, presence, search, etc.) are untouched. Drafts never enter `state.chats`, so live socket events (`new_message` etc.) can't accidentally target them.
  - **Unread divider centering — finished restructure.** "N unread messages" pill is now emitted as a top-level synthetic `senderId: 'unread'` group from `formattedChatData()` (same pattern as date / system separators), instead of injected inside the per-message map nested inside a speaker's chat-body Box. Result: the pill spans the FULL chat width and centers correctly, no longer constrained by the sender column's 65–75% `maxWidth`. The `unreadDividerRef` is still attached to the same divider element (just at a different tree depth), so the auto-scroll-to-divider effect, the load-older cooldown (1500ms pre-arm + 800ms reinforce), the own-send forced scroll, the near-bottom live-update gating, and the jump-load-when-anchor-outside-window — ALL preserved byte-equivalent. Side benefit: messages straddling the unread boundary now break into two speaker rows (avatar repeated), matching WhatsApp Web's behavior.
  - **`formatLastSeen` extracted to shared util.** New [`src/lib/chat/formatLastSeen.ts`](../../../src/lib/chat/formatLastSeen.ts) — single source of truth for the WhatsApp-style "last seen X" copy. Consumed by [`ChatContent`](../../../src/views/apps/chat/ChatContent.tsx) (chat header) + [`UserProfileRight`](../../../src/views/apps/chat/UserProfileRight.tsx) (DM profile drawer). Removed 23-line + 22-line duplicate copies from the two files. Format switched from `hour: '2-digit'` (locale-dependent — could render 24h on en-GB / 12h on en-US) to explicit `hour: 'numeric', hour12: true` so output is identical across all browser locales: `"last seen today at 2:30 PM"` / `"last seen yesterday at 11:05 AM"` / `"last seen 12/04/2026 at 9:14 PM"`.

- **2026-05-22** — Teal-palette migration across chat surfaces + presence end-to-end (cold-seed + live `user_offline` REST refetch + per-room dedupe) + socket lifecycle timestamps + group-icon visual consistency across all surfaces:
  - **Group icon visual consistency.** The teal-gradient circle + white `mdi:account-group` glyph from the group-created card in [`ChatLog`](../../../src/views/apps/chat/ChatLog.tsx) (gradient: `secondary.light` → `secondary.main`) is now used in all four surfaces that render a group's avatar when no `iconUrl` has been uploaded:
    - [`SidebarLeft`](../../../src/views/apps/chat/SidebarLeft.tsx) chat-row group avatar (40×40)
    - [`ChatContent`](../../../src/views/apps/chat/ChatContent.tsx) chat-header group avatar (40×40)
    - [`ComposePopover`](../../../src/views/apps/chat/ComposePopover.tsx) "New group" CTA icon (36×36) — also fixed a hover bug where the icon color flipped to white but the circle background stayed white, making it invisible
    - [`ChatLog`](../../../src/views/apps/chat/ChatLog.tsx) group-created card (72×72) — pre-existing source visual
  - **Send button + composer hovers → secondary palette.** [`SendMsgForm`](../../../src/views/apps/chat/SendMsgForm.tsx) send IconButton bg flipped from `primary.main` → `secondary.main` (hover `secondary.dark`); attachment paperclip + mic icons now hover to `secondary.main` instead of `primary.main`. Send icon wrapped in a `Box` with `transform: translateX(1px)` so the asymmetric `mdi:send` glyph reads visually centered inside the 42px circle (geometrically centered but visual weight skews left).
  - **AvatarUpload component palette swap.** [`AvatarUpload`](../../../src/views/forms/form-elements/file-uploader/AvatarUpload.tsx) — used by the "Add icon" placeholder in [`CreateGroupDrawer`](../../../src/views/apps/chat/CreateGroupDrawer.tsx) — gradient + soft glow + drag-active / hover shadows all flipped from `primary` → `secondary` so the empty-state circle reads as teal-family. Verified `AvatarUpload` has only one consumer (CreateGroupDrawer), so the swap is safe.
  - **CreateGroupDrawer entire component on secondary palette.** All `primary.*` references swapped: "{N} added" pill text `primary.dark` → `secondary.dark`; selected-avatar overlay bg `primary.main` → `secondary.main`; trailing check-circle next to selected name `primary.main` → `secondary.main`; "Create Group" Button gains `color='secondary'` (teal contained-bg + secondary.dark hover); group name + description TextField focused `:after` underline added with `secondary.main` color (was falling through to MUI's default primary green).
  - **Search field hover/focus border pattern** standardized across all "Search people" / "Search name or staff ID" / "Search" surfaces ([`CreateGroupDrawer`](../../../src/views/apps/chat/CreateGroupDrawer.tsx), [`AddMembersDrawer`](../../../src/views/apps/chat/AddMembersDrawer.tsx), [`UserProfileRight`](../../../src/views/apps/chat/UserProfileRight.tsx) add-members panel, [`ComposePopover`](../../../src/views/apps/chat/ComposePopover.tsx), [`SidebarLeft`](../../../src/views/apps/chat/SidebarLeft.tsx)): transparent background, `secondary.main` border at 0.5px rest → 2px hover/focus with 160ms transition. Light-green `customColors.Surface` backgrounds removed entirely. Hardcoded `#1F515B` border colors replaced with `secondary.main` theme token throughout (matches Create Group button color so the same shade reads across button + search fields).
  - **Sidebar chat-row hover → secondary.main + white text.** [`SidebarLeft`](../../../src/views/apps/chat/SidebarLeft.tsx) `ListItemButton` `'&:hover'` rule replaces MUI's default neutral gray overlay with a `secondary.main` background + descendant `Typography` color flip to `common.white`. Active rows keep their dark `#1F515B !important` (the `!important` ensures hover doesn't override).
  - **"New chat" IconButton hover** in [`SidebarLeft`](../../../src/views/apps/chat/SidebarLeft.tsx) header now uses `secondary.main` theme token instead of hardcoded `#1F515B`.
  - **Member-list row hover stays neutral.** [`CreateGroupDrawer`](../../../src/views/apps/chat/CreateGroupDrawer.tsx) member rows use `action.hover` (default MUI neutral) — selection is communicated by the teal avatar overlay + trailing check-circle, NOT by row-bg color. Per UX request: "selected" should be the dominant visual cue, not "hover".
  - **Presence end-to-end (DM only — groups have no presence by spec).** Diagnosed via runtime logs and stabilized through three coordinated paths:
    - **Online cold-seed** — restored [`AppChat.tsx`](../../../src/views/apps/chat/AppChat.tsx) effect that fires `socketEmit.getOnlineUsers([all DM peer ids])` once per socket-connect (ref-gated, resets on disconnect). Set-merged into `useChatStore.onlineUsers` so any `user_online` events arriving during the round-trip aren't clobbered. Without this, page refresh while peer is online would render "last seen X" because `useChatStore.onlineUsers` starts empty (in-memory only, no persistence) and the server doesn't auto-push `user_online` snapshot events for already-connected peers on a fresh socket.
    - **`lastSeen` cold-seed** — REST `getUserLastSeen(peerId)` fires once per DM open in [`ChatContent.tsx`](../../../src/views/apps/chat/ChatContent.tsx) + once per profile-drawer open in [`UserProfileRight.tsx`](../../../src/views/apps/chat/UserProfileRight.tsx). Both gate on `if (lastSeenMap[peerId]) return` for idempotence — once a value lands (from REST or live socket event), no refetches.
    - **Live `user_offline` REST refetch** — new listener in [`AppChat.tsx`](../../../src/views/apps/chat/AppChat.tsx) catches the live event, refetches via `getUserLastSeen(userId)` to grab the latest server value, and writes into `useChatStore.lastSeen`. Workaround for the backend gap where `user_offline` payload omits `lastSeenAt` (see Known server-side quirks #11). `evt.lastSeenAt ?? <REST refetch>` so the day the server starts including the field, no further client change is needed. Also dedupes the per-room 5×-fanout (server emits one event per joined room within ~80ms — we swallow duplicates inside a 1s window per userId via `lastOfflineFetchedAtRef`).
  - **Socket lifecycle logs gain timestamps.** [`socketLogger.ts`](../../../src/lib/chat/socketLogger.ts) `[chat:socket] ✅ CONNECTED` / `❌ DISCONNECTED` now include `at: <local time> (<ISO>)` so console logs can be cross-referenced with `[chat:lastSeen]` / `[chat:receipt]` traces. Reason field on disconnect is the standard Socket.IO cause (`io client disconnect` / `ping timeout` / `transport close` / `transport error`).
  - **Backend gaps documented** as items #11 and #12 in the Known server-side quirks section above:
    - `user_offline` event payload missing `lastSeenAt`
    - `users.lastSeenAt` reflects last activity time, not actual socket-disconnect time

- **2026-05-21b** — WhatsApp-Web search drawer + standalone AddMembersDrawer + starred click-to-jump + attachment-only star indicator + small-screen search-icon fix + **WhatsApp-Web "N unread messages" divider** + ChatLog scroll polish (own-send detection, near-bottom live-update gating, load-older cooldown):
  - **WhatsApp-Web right-side message search drawer** — new [`SearchMessagesDrawer`](../../../src/views/apps/chat/SearchMessagesDrawer.tsx) replaces the inline search bar previously embedded in the chat header. Controlled component (parent owns `searchOpen` / `searchQuery` / `searchResultIds` / `activeMatchIndex` state). Owns its own debounced query → `searchMessages(conversationId, query)` REST call, hit-list row rendering with `<mark>`-highlighted match substring, and result count footer. Clicking a row scrolls the bubble into view via the existing `onJumpToMessage` (loads context window if outside current page) and highlights the active match — same path as the prev/next arrow navigation. Sidebar primitive uses `position: absolute` so it mounts at the chat shell root — `ChatContent` now gates with `{searchOpen && <SearchMessagesDrawer ... />}` (an always-mounted child caused a blank screen). Drawer closes without reloading the chat — matches WhatsApp Web (the earlier `selectChat(chatId)` "snap back to latest" dispatch was removed).
  - **Starred-message click-to-jump.** [`StarredMessagesDrawer`](../../../src/views/apps/chat/StarredMessagesDrawer.tsx) accepts a new optional `onMessageClick?: (messageId: string) => void` prop, wired up at the row `onClick`. [`UserProfileRight`](../../../src/views/apps/chat/UserProfileRight.tsx) renders the drawer with `onMessageClick={onScrollToMessage}` so clicking a starred row routes through the same `setScrollTargetMessageId` → `onJumpToMessage` path the search drawer and pinned-bar use — context window loads if the message is outside the current page, then the bubble flashes via `.msg-flash`. New optional `UserProfileRightType.onScrollToMessage?: (messageId: string) => void` exposed for parents to wire.
  - **Standalone `AddMembersDrawer`** ([`src/views/apps/chat/AddMembersDrawer.tsx`](../../../src/views/apps/chat/AddMembersDrawer.tsx)) — pulled out of the group-created card flow into its own right-side drawer that mounts at the chat shell root (same Sidebar primitive pattern as Group info / Search / Starred drawers). Triggered by the "Add Member" button on the group-created card (`ChatLog` already exposed `onAddMember` prop; `ChatContent` now sets it to a new `addMembersDrawerOpen` state setter). Owns its own state — query, debounced `searchUsers`, selected ids set, "Add as admin" Switch, contacts list — and dispatches `addParticipantsToGroup({ chatId, userIds, role })`. Initial attempt used a `useEffect` "signal" pattern inside `UserProfileRight` (`openWithAddMembers` prop + `onAddMembersConsumed` callback) which produced a blank-screen mount race; the standalone drawer cleanly replaces it. The pre-existing "Add members" panel inside `UserProfileRight` Group info is unchanged.
  - **Star indicator on attachment-only bubbles** — `ChatLog` text bubbles already showed a `mdi:star` next to the time row when `chat.isStarred`; the attachment-only render path (audio / video / image / doc with no caption) was missing it. Added a matching span next to `renderMsgFeedback(...)` in the timestamp row at the bottom of the attachment column. Styling mirrors the pin indicator: `color: 'inherit'`, `fontSize: '1rem'`, no manual opacity — so it reads the same against either sender (green/white) or receiver (paper) bubble surfaces. Also flipped the "Unstar" menu item's icon in [`MessageActions`](../../../src/views/apps/chat/MessageActions.tsx) from `mdi:star-outline` → `mdi:star-off-outline` to mirror the existing `pin-outline` ↔ `pin-off-outline` pattern.
  - **Search icon visibility on small screens.** [`ChatContent`](../../../src/views/apps/chat/ChatContent.tsx) previously hid the search IconButton inside an `{mdAbove ? <…> : null}` gate (shared with the desktop-only group-icon left rail). Moved the search button out of that gate so it renders at every breakpoint — mobile users can now open the search drawer from the header. The `mdAbove`-gated rail keeps the avatar / contact-name layout split as before.
  - **WhatsApp-Web "N unread messages" divider.** New centered pill rendered inline in the message stream at the boundary between read and unread messages, in [`ChatLog`](../../../src/views/apps/chat/ChatLog.tsx).
    - **Anchor source.** SDK auto-seeds `useChatStore.lastRead[conversationId] = { messageId, readAt, deviceId }` via the `getLastRead(chatId)` call already wired in `selectChat` thunk. `ChatLog` subscribes via `useChatStore(s => s.lastRead)`.
    - **Snapshot once per chat open.** A `useRef` + `useState` pair captures `lastRead.messageId` the FIRST time it appears for the current chat. After that the snapshot is frozen — `selectChat`'s subsequent `markReadOverSocket` flips `useChatStore.lastRead` to "latest" and would otherwise erase the divider mid-session.
    - **First-unread resolution.** Computed every render via `data.chat.messages.findIndex(m => m.id === anchor)` + a forward walk picking the first message whose `senderId !== userContact.id` (matches WhatsApp — divider only meaningful before someone else's bubble, not your own send).
    - **Divider injection.** Inside `renderChats()` the per-message map carries a `unreadDividerRendered` flag; the divider Fragment is emitted immediately before the bubble whose id matches `firstUnreadInfo.id`. Exactly once per render no matter how many sender-grouped runs the unread block spans. Pill styling reuses the same `theme.palette.action.hover` background + `text.secondary` color + caption variant as the existing date / system separators.
    - **Gap-1 auto-scroll.** Direct `unreadDividerRef` + custom `smoothScrollTo(target)` lands the divider at the top of the viewport (`block: 'start'`) — bubble-centering would have pushed the divider off-screen above. Guarded by a `scrolledToUnreadRef` so it fires exactly once per anchor.
    - **Gap-2 jump-load.** When `lastRead.messageId` is OLDER than the loaded window (cursor pagination keeps only ~50 messages around the live position), `findIndex` returns -1 → effect dispatches `onJumpToMessage(anchor)` exactly once via `requestedUnreadJumpRef`. Same pattern as the search/pinned-bar jump paths — context window resolves, effect re-runs, divider mounts, scroll lands. Defensive `if (msgs.length === 0) return` prevents a spurious jump while `listMessages` is still in flight (otherwise the about-to-arrive normal page would be replaced by a context window, looking like a chat-reload right after open).
  - **ChatLog scroll-effect polish around the unread divider.**
    - **Own-send always scrolls to bottom.** The data effect now reads the newest message's `senderId`. If it equals the current user's id, `scrollToBottom(true)` runs with a force flag — bypasses all unread-divider holds. WhatsApp parity: sending a message from the divider position should land you at the bottom so you see your own message.
    - **Live updates only follow when at/near bottom.** For incoming messages from other senders (after the initial render has consumed `firstRenderForChatRef`), the effect checks `scrollHeight - scrollTop - clientHeight`. If the user has scrolled up more than 200px from the bottom, the scroll-to-bottom is skipped — the existing scroll-to-bottom FAB indicator (already gated on `distFromBottom > 300`) handles the unread cue instead. Matches WhatsApp Web: a new message while reading older history doesn't yank you down.
    - **Initial render path.** New `firstRenderForChatRef` ref (set to `selectedChatId` in the chat-switch reset effect, consumed on first non-empty `messages` render). On initial render with known unread → defer to the unread-scroll effect; otherwise → `scrollToBottom()` as before.
    - **Load-older cooldown.** PerfectScrollbar fires `onYReachStart` as a side-effect of our own `smoothScrollTo` landing the divider near the top of the viewport (most visible in short DMs where there are few messages above the divider). Without a guard, PSB calls `triggerLoadOlder` → REST page prepends → view shifts upward right after the divider lands. New `ignoreLoadOlderUntilRef` (ms timestamp) checked at the top of `triggerLoadOlder`. Pre-armed (1500ms window) inside the snapshot effect the instant we know we'll be landing near the top — guarantees the ref is set BEFORE any PSB event could fire. Reinforced (800ms) inside the unread-scroll effect after the actual `smoothScrollTo`.
    - **Time-windowed `doScroll` inner guard.** `scrollToBottom` queues four passes (rAF / rAF / setTimeout 120ms / setTimeout 350ms) to catch late-loading content. Any callback still pending when the unread-scroll lands would yank the user past the divider. Inner check is now `if (!force && scrolledToUnreadRef.current && Date.now() - unreadScrollAtRef.current < 1000) return` — blocks queued callbacks racing against the unread-scroll, but lifts after 1s so genuine new-message scroll-to-bottom calls flow normally. `force` (own-send) bypasses unconditionally.
    - **SDK-store flicker guard** (in initial-render branch only): if `useChatStore.lastRead[chatId].messageId !== newestId` we skip the initial `scrollToBottom` and wait for the snapshot effect → unread-scroll path. Avoids the flicker where the chat briefly lands at bottom before the snapshot/unread-scroll path runs on a later render.
